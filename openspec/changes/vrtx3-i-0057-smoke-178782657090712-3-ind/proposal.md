# Three independent health probes for variant 436511294
...
endpoints — `/api/healthz-smoke-436511294-a`, `-b` and `-c` — are blocked on that overhead.
...
- **ADDED** `GET /api/healthz-smoke-436511294-a`, returning `{"ok": true, "variant": "436511294"}`
  with a JSON content type.
- **ADDED** `GET /api/healthz-smoke-436511294-b`, same contract.
- **ADDED** `GET /api/healthz-smoke-436511294-c`, same contract.
...
build configuration, Playwright coverage, and the 145 existing `healthz-smoke-*` routes —
