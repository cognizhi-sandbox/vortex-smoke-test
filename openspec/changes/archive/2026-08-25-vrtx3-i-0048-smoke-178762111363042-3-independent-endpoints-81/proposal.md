# Three independent health probes for variant 812788042
...
endpoints — `/api/healthz-smoke-812788042-a`, `-b` and `-c` — are blocked on that overhead.
...
- **ADDED** `GET /api/healthz-smoke-812788042-a`, returning `{"ok": true, "variant": "812788042"}`
  with a JSON content type.
- **ADDED** `GET /api/healthz-smoke-812788042-b`, same contract.
- **ADDED** `GET /api/healthz-smoke-812788042-c`, same contract.
...
build configuration, and the 121 existing `healthz-smoke-*` routes.
