# Design — three independent health probes (992401223)

## Context

`routes/api/` holds 239 files at the time of writing: 118 probe handlers, their 118 colocated
tests, and 6 worked-example files (`hello.ts`, `hello.test.ts`, four under `users/`). A repo-wide
grep for `992401223` returns zero matches, so all three target paths are unwritten.

Measured live on a dev server during planning (Vite bound `:5000`; read your own banner, the port
is per-container):
...
and would turn this change from six new files into a refactor of 118 existing routes.
...
`/api/healthz-smoke-992401223-a` → `healthz_smoke_992401223_a.mjs`).

### D4 — Copy `healthz-smoke-528856326-a.{ts,test.ts}`, not the file the idea names

47 of the 118 existing probe tests, all written before VRTX3-S-0011, carry a second
...
VRTX3-I-0047 names `healthz-smoke-189360772-a` as its template. That pair was diffed during
planning and carries no timing case, so the substitution costs nothing on this change — but it is
applied regardless, because the directory offers no in-band way to tell a safe neighbour from a
legacy one, and three prior changes in this family have propagated the timing case by sampling.

### D5 — No structural guard against non-`GET` verbs

None of the 118 existing handlers declares a method guard, so every verb receives the same body.
...
```ts
// routes/api/healthz-smoke-992401223-a.ts
...
```ts
// routes/api/healthz-smoke-992401223-a.test.ts
...
value is the literal string `"992401223"` on all three probes rather than a per-probe value.
