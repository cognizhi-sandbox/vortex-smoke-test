# Three independent health probes for variant 865643533
...
endpoints — `/api/healthz-smoke-865643533-a`, `-b` and `-c` — are blocked on that overhead.
...
- **ADDED** `GET /api/healthz-smoke-865643533-a`, returning `{"ok": true, "variant": "865643533"}`
  with a JSON content type.
- **ADDED** `GET /api/healthz-smoke-865643533-b`, same contract.
- **ADDED** `GET /api/healthz-smoke-865643533-c`, same contract.
...
build configuration, Playwright coverage, and the 127 existing `healthz-smoke-*` routes —
