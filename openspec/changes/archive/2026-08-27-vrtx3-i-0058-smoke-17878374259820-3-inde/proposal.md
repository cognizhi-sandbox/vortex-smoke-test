# Three independent health probes for variant 956166896
...
endpoints — `/api/healthz-smoke-956166896-a`, `-b` and `-c` — are blocked on that overhead.
...
- **ADDED** `GET /api/healthz-smoke-956166896-a`, returning `{"ok": true, "variant": "956166896"}`
  with a JSON content type.
- **ADDED** `GET /api/healthz-smoke-956166896-b`, same contract.
- **ADDED** `GET /api/healthz-smoke-956166896-c`, same contract.
...
build configuration, Playwright coverage, and the 148 existing `healthz-smoke-*` routes —
...
- **Risk:** low and bounded. The diff is additive, so there is no regression path into current
  ...
  No root document is touched, because none of the three update triggers fires — see the
  technical decisions in this change, § D4.
