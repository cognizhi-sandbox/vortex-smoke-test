# Technical decisions — three health probes for variant 956166896
...
Taken on `vortex/sprint/vrtx3-s-0048-aaf68415` at `01ae675` during planning.

| Measurement                                                  | Value |
| ------------------------------------------------------------ | ----- |
| `healthz-smoke-*` handlers under `routes/api/`               | 148   |
| Colocated `healthz-smoke-*.test.ts` files                    | 148   |
| Probe tests carrying `expect(elapsed).toBeLessThan(100)`     | 47    |
| `.ts` files under `routes/api/` (recursive)                  | 302   |
| Test files across `src/` and `routes/` (pre-sprint baseline) | 155   |
| Files matching `956166896`                                   | 0     |

The last row is the one this change acts on. The 47 legacy tests are never rewritten, so that
numerator is fixed while the family grows — see § D2. The idea canvas cites "299 files in
`routes/api/`", which was accurate when the idea was written and drifted by the three probes
VRTX3-S-0047 landed; nothing in the change depends on the figure.
...
| Path                                       | Result                                                                 |
| ------------------------------------------ | ---------------------------------------------------------------------- |
| `/api/healthz-smoke-956166896-a`           | `200 text/html; charset=utf-8` — the SPA shell                         |
| `/api/healthz-smoke-956166896-b`           | `200 text/html; charset=utf-8` — the SPA shell                         |
| `/api/healthz-smoke-956166896-c`           | `200 text/html; charset=utf-8` — the SPA shell                         |
| `/api/healthz-smoke-528856326-a` (control) | `200 application/json;charset=UTF-8 {"ok":true,"variant":"528856326"}` |
| `POST /api/healthz-smoke-528856326-a`      | `200 application/json;charset=UTF-8 {"ok":true,"variant":"528856326"}` |
...
`Content-Type`. The idea canvas made no status-code claim — it says only that "no file matching
`956166896` exists yet", which is true and reproducible — so there was nothing to correct this
time; the measurement was taken regardless, because only it says what is on disk today.

The `POST` row confirms the canvas's own edge-case note and its AC-8: handlers here declare no
method guard, so every verb returns the same body. That is the existing family-wide behaviour and
this change keeps it — it is not a defect and not something to "fix" on three routes in isolation.
...
the evidence, and the idea's third user story asks for it explicitly.
...
sibling as the copy source. 47 of the 148 probe tests predate VRTX3-S-0011 and carry a second
...
**What this idea named, and how it checks out.** VRTX3-I-0058 cites the
`healthz-smoke-1065915107-{a,b,c}` set as the sibling pattern, and its Affected Code section
narrows that to `healthz-smoke-1065915107-{a,c}.ts` / `.test.ts`. Both tests were diffed at
planning: neither contains `toBeLessThan`, both are post-VRTX3-S-0011, both are a single body
assertion. The citation is clean — another harmless instance of the sampling hazard. No ordinal is
asserted for it: the running counts in `AGENTS.md` and in prior sprint summaries are maintained
independently and already disagree, so the recurrence is recorded and the numbering is not.

Two things follow, and neither relaxes the rule. The set `1065915107` is the same one
VRTX3-I-0048 cited in VRTX3-S-0039, so a repeat citation is not independent evidence — it is the
same neighbour being sampled twice. And the substitution to the pinned pair is made anyway and
costs nothing: the check that a pointer is right is the one diff that catches a wrong one, so
reading the named file is never skippable. Implement from the `528856326-a` pair and change only
the variant string and the identifier names.
...
- Module: `routes/api/healthz-smoke-956166896-<letter>.ts`
- Only import: `defineHandler` from `nitro/h3`
- Default export: `defineHandler(() => ({ ok: true, variant: "956166896" }))` — a parameterless
  arrow returning an object literal, `variant` a **string**
- No `event` parameter read, no method guard, no `db/` import, no sibling import

The colocated test is `routes/api/healthz-smoke-956166896-<letter>.test.ts`: build
`new H3Event(new Request("http://localhost/api/healthz-smoke-956166896-<letter>"))`, `await` the
default export, and `expect(result).toEqual({ ok: true, variant: "956166896" })`. One `it` block.

The variant string `956166896` is shared by all three probes and the trailing `-a`/`-b`/`-c`
distinguishes the paths — matching how existing sibling sets behave (`healthz-smoke-1065915107-a`
and `-c` both return `variant: "1065915107"`).
...
- **`DESIGN.md`** — no token, type scale, grid, interaction pattern or accessibility standard
  moves. There is no UI in this change; `a2a_get_idea_design(VRTX3-I-0058)` returns an empty
  manifest, so nothing was exported to `artifacts/VRTX3-S-0048/design/`.

`AGENTS.md` is human-authored and is never rewritten. Its probe-family denominator has drifted
again (it says 124; the filesystem says 148). `.vortex/agents-generated.md` already records that
...
- A route's unit test imports the handler module directly, so it passes even if Nitro never
  ...
  `.output/server/_routes/api/healthz_smoke_956166896_<letter>.mjs` (dashes become underscores).
...
Expected post-sprint test-file total: 158 (baseline 155 + 3).
...
no edit. The workflow runs doc-links, typecheck, lint, the unit tier and the build; three new
