...
The system SHALL serve `GET /api/healthz-smoke-956166896-a` with HTTP 200, a JSON content type,
and a response body deep-equal to `{"ok": true, "variant": "956166896"}`. The response MUST NOT
depend on the request's query string, headers, body, authenticated user or any stored data, and
the handler MUST NOT import any sibling probe or any database module.
...
- **GIVEN** the service is running
- **WHEN** a client sends `GET /api/healthz-smoke-956166896-a`
- **THEN** the response status is 200, its `Content-Type` is `application/json`, and its body is
  deep-equal to `{"ok": true, "variant": "956166896"}` with `variant` as a string

#### Scenario: Repeat calls return byte-identical JSON

- **GIVEN** the service is running
- **WHEN** a client sends two successive requests to `/api/healthz-smoke-956166896-a` that differ
  in query string, headers and request body
...
- **GIVEN** the file `routes/api/healthz-smoke-956166896-a.ts`
- **WHEN** its imports and handler body are inspected
- **THEN** its only import is `defineHandler` from `nitro/h3`, it reads no property of the event,
  and it references no sibling probe and no module under `db/`
...
- **GIVEN** the file `routes/api/healthz-smoke-956166896-a.test.ts`
- **WHEN** the unit test tier runs
- **THEN** it constructs an `H3Event` for the probe path, invokes the module's default export,
  asserts the result equals `{ ok: true, variant: "956166896" }`, and contains no wall-clock
  timing assertion
...
- **GIVEN** a production build of the server
- **WHEN** its route output is inspected
- **THEN** it contains the module `.output/server/_routes/api/healthz_smoke_956166896_a.mjs` and
  contains no `.test.ts` file

### Requirement: Health probe B for variant 956166896

The system SHALL serve `GET /api/healthz-smoke-956166896-b` with HTTP 200, a JSON content type,
and a response body deep-equal to `{"ok": true, "variant": "956166896"}`. The response MUST NOT
depend on the request's query string, headers, body, authenticated user or any stored data, and
the handler MUST NOT import any sibling probe or any database module.
...
- **GIVEN** the service is running
- **WHEN** a client sends `GET /api/healthz-smoke-956166896-b`
- **THEN** the response status is 200, its `Content-Type` is `application/json`, and its body is
  deep-equal to `{"ok": true, "variant": "956166896"}` with `variant` as a string
...
- **GIVEN** the service is running
- **WHEN** a client sends two successive requests to `/api/healthz-smoke-956166896-b` that differ
  in query string, headers and request body
...
- **GIVEN** the file `routes/api/healthz-smoke-956166896-b.ts`
- **WHEN** its imports and handler body are inspected
- **THEN** its only import is `defineHandler` from `nitro/h3`, it reads no property of the event,
  and it references no sibling probe and no module under `db/`
...
- **GIVEN** the file `routes/api/healthz-smoke-956166896-b.test.ts`
- **WHEN** the unit test tier runs
- **THEN** it constructs an `H3Event` for the probe path, invokes the module's default export,
  asserts the result equals `{ ok: true, variant: "956166896" }`, and contains no wall-clock
  timing assertion
...
- **GIVEN** a production build of the server
- **WHEN** its route output is inspected
- **THEN** it contains the module `.output/server/_routes/api/healthz_smoke_956166896_b.mjs` and
  contains no `.test.ts` file

### Requirement: Health probe C for variant 956166896

The system SHALL serve `GET /api/healthz-smoke-956166896-c` with HTTP 200, a JSON content type,
and a response body deep-equal to `{"ok": true, "variant": "956166896"}`. The response MUST NOT
depend on the request's query string, headers, body, authenticated user or any stored data, and
the handler MUST NOT import any sibling probe or any database module.
...
- **GIVEN** the service is running
- **WHEN** a client sends `GET /api/healthz-smoke-956166896-c`
- **THEN** the response status is 200, its `Content-Type` is `application/json`, and its body is
  deep-equal to `{"ok": true, "variant": "956166896"}` with `variant` as a string
...
- **GIVEN** the service is running
- **WHEN** a client sends two successive requests to `/api/healthz-smoke-956166896-c` that differ
  in query string, headers and request body
...
- **GIVEN** the file `routes/api/healthz-smoke-956166896-c.ts`
- **WHEN** its imports and handler body are inspected
- **THEN** its only import is `defineHandler` from `nitro/h3`, it reads no property of the event,
  and it references no sibling probe and no module under `db/`
...
- **GIVEN** the file `routes/api/healthz-smoke-956166896-c.test.ts`
- **WHEN** the unit test tier runs
- **THEN** it constructs an `H3Event` for the probe path, invokes the module's default export,
  asserts the result equals `{ ok: true, variant: "956166896" }`, and contains no wall-clock
  timing assertion
...
- **GIVEN** a production build of the server
- **WHEN** its route output is inspected
- **THEN** it contains the module `.output/server/_routes/api/healthz_smoke_956166896_c.mjs` and
  contains no `.test.ts` file
