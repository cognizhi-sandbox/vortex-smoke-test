...
**Note the sprint key: this is the SECOND sprint to run as `VRTX3-S-0002`, with the same three ticket keys as the first.** The 2026-08-02 entry further down this changelog is a different sprint (variants `106285986`, `524723214`, `764107669`) that reused `VRTX3-T-0007/0008/0009`. Its artifacts were already on disk at `artifacts/VRTX3-S-0002/` from commit `e167bb8`, so planning **overwrote** `SPRINT-PLAN.md` and the three `PLAN.md` files and left the rest — `fix-note.md`, `tdd-test-result.md`, `qa-test-report.md`, `release-notes.md`, `sprint-summary.md`, `integration-test-result.md` — in place, since deleting another sprint's record was out of scope. The hazard is specific and worth knowing if keys recycle again: mid-sprint, each per-ticket directory held a stale `fix-note.md` reporting a **completed** fix for a variant that was not the current ticket's, which reads as "already done" to anyone who opens the directory rather than the plan. Every `PLAN.md` was given a warning banner naming the stale files.

**By close, nothing stale was left — but only by luck, so do not rely on it.** Every stale file happened to be overwritten in the normal course of the sprint: implementation replaced the three `fix-note.md` and `tdd-test-result.md` files, validation replaced `qa-test-report.md` and `integration-test-result.md`, and the close bundle replaced `release-notes.md` and `sprint-summary.md`. That worked because this sprint regenerated every artifact type the directory already held. A sprint producing a different artifact set — or one that closes without a full bundle — would leave another sprint's files sitting beside its own, and the recycled key gives no signal that has happened. If you find yourself planning into a directory that already exists, read `git log` on it before trusting anything in it.
