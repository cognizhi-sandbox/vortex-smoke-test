...
- **A missing `/api/*` route returns `200 text/html`, NOT `404`.** An unmatched API path falls through to the SPA `index.html` shell, in `bun run dev` and in the production build alike (nginx does not change this — `location /api/` proxies straight to Nitro with `proxy_intercept_errors` off). So **status code alone cannot tell a working endpoint from a missing one**, and a `404 → 200` check proves nothing. When adding or verifying an API route, assert on the **response body and `Content-Type`**:
  ...
  **The status code is not the only field defect capture mis-transcribes — VRTX3-S-0029 lost the `/api/` prefix too.** Both its committed defects named the probe as `/healthz-smoke-bugfix-ha-971401638`, at the URL root, where every prior bugfix ticket had written `/api/healthz-smoke-bugfix-27681476`. **Resolve it in favour of `routes/api/`, not the report.** `routes/` contains exactly one entry, `api/` — `find routes -name "*.ts" -not -path "routes/api/*"` returns nothing, so no handler has ever lived at the root — and the per-probe contract is fixed as `GET /api/<probe-name>` in [PRODUCT.md](./PRODUCT.md) § Features and as `routes/api/x.ts` → `/api/x` in [ARCHITECTURE.md](./ARCHITECTURE.md#routing). This changelog does the same thing to itself: the SPRINT-0004/-0005/-0007 entries below name `/healthz-smoke-cancel-407995880`, whose file is and always was `routes/api/healthz-smoke-cancel-407995880.ts`. Serving the bare path would mean a root-level `routes/*.ts` contradicting two root docs to satisfy a field from the channel that also reports `404`. **Measuring cannot settle this one** — a missing route answers `200 text/html` at both spellings, so both look identical before the fix and only the prefixed one is right after it.
...
Added two missing health probes, each returning HTTP 200 with `Content-Type: application/json` and body `{ ok: true, variant: "<id>" }`: `/api/healthz-smoke-bugfix-ha-971401638`, `/api/healthz-smoke-bugfix-ha2-649579386`. Purely additive — 4 new files, 0 existing source files modified. Probe family count 95 → 97, re-derived from the filesystem and bumped in all three docs that carry it (`AGENT.md`, `ARCHITECTURE.md`, `PRODUCT.md`) in the same pass. Note the sprint is two defects, not the usual three, and the family stem is new (`bugfix-ha`, `bugfix-ha2`) — `-ha`/`-ha2` is a name, not a grouping, and the two probes share no file.

**A second field of the defect report was wrong this sprint, and it is one no measurement can settle.** Both tickets named their probe at the URL root — `/healthz-smoke-bugfix-ha-971401638` — dropping the `/api/` prefix that every prior bugfix ticket carried (VRTX3-T-0167's title reads `/api/healthz-smoke-bugfix-27681476`). The status-code drift is self-correcting because a live request exposes it; this one is not, because a missing route answers `200 text/html` at **both** spellings. It was resolved from the repository instead: `routes/` holds only `api/`, and the per-probe URL contract is fixed in `PRODUCT.md` § Features and `ARCHITECTURE.md` § Routing. [Gotchas](#gotchas) now records it alongside the `404`, with the note that this changelog commits the same error about its own `cancel-*` probes.

**Twentieth consecutive confirmation of the SPA-fallback trap, and the first taken at two spellings of each path.** Re-measured on a live dev server during planning rather than cited: all four candidate paths (`/api/`-prefixed and bare, for both probes) returned `200 text/html; charset=utf-8` (the SPA shell, 949 bytes), while the control `/api/healthz-smoke-528856326-a` returned `200 application/json;charset=UTF-8` (33 bytes). A repo-wide grep for `971401638`, `649579386` and the sprint tag `178724114989195` returned zero matches, confirming never-written files rather than typo'd filenames.

**Neither defect has an idea linked at all** — `a2a_get_idea_canvas` and `a2a_get_idea` both returned "not linked to an idea". This is the fully-ungrounded end of the uneven-capture split that VRTX3-S-0018, -0020 and -0024 each hit with a partial canvas: there was no upstream reasoning to check, correct or credit, and both the `404` and the missing prefix arrived unchecked. Nothing about planning changes — re-measure, whatever the report says — but it is the first sprint where nothing upstream was available to re-measure _against_.

**Vite bound `:5001`** (`5000` in use). Twelve sprints have now produced `:5005`, `:5006`, `:5007`, `:5000`, `:5001`, `:5002`, `:5000`, `:5000`, `:5000`, `:5000`, `:5000` and `:5001` — read the banner.

The `528856326` copy-source pointer had nothing to contend with this sprint: with no idea behind either ticket, no template was named at all, so the pointer applied by default. The flaky `responds in under 100ms` case remains confined to the 47 pre-VRTX3-S-0011 tests, now 47 of 97.
