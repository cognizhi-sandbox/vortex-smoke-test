...
VRTX3-S-0003 made it three harmless to one: VRTX3-I-0006 named `healthz-smoke-bugfix3-834560860.test.ts`, the pair added by the immediately preceding sprint, so it was shape-identical by construction and the substitution again cost nothing. Note what makes this one worth recording rather than routine — the canvas did not sample at random, it named the **most recent** probe, which is the sampling heuristic most likely to be safe and therefore most likely to look like a reason to stop substituting. It is not one. The ratio (now 47 of 106) is fixed by files that are never rewritten, and the next canvas that reaches one directory entry further back lands in the legacy half.
