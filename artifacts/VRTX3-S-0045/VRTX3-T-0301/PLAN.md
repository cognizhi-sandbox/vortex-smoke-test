...
`/api/healthz-smoke-bugfix-1022589408` returns the SPA shell, should return `ok` + `variant`.

Change: `vrtx3-s-0045-smoke-bugfix-sprint-smoke-b`
Requirement: **Health probe for bugfix variant 1022589408** (`specs/health-probes/spec.md`)

## Objective

Serve `GET /api/healthz-smoke-bugfix-1022589408` with `200 application/json` and the body
`{"ok":true,"variant":"1022589408"}`, plus a colocated regression test.
...
1. Create `routes/api/healthz-smoke-bugfix-1022589408.ts` from the pinned template in `design.md`
   § D2, with `variant: "1022589408"`. Copy the pinned `healthz-smoke-528856326-a.ts` — not a
   directory neighbour, and not the sibling any idea names (`design.md` § D2 explains why).
2. Create `routes/api/healthz-smoke-bugfix-1022589408.test.ts` from the same section's test
...
| File                                                 | Action |
| ---------------------------------------------------- | ------ |
| `routes/api/healthz-smoke-bugfix-1022589408.ts`      | create |
| `routes/api/healthz-smoke-bugfix-1022589408.test.ts` | create |

Nothing else. No existing file is modified, and no shared helper, factory, constants module or
barrel export is introduced (`design.md` § D4). Ownership is disjoint from VRTX3-T-0302 and
