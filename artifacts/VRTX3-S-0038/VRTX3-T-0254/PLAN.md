...
# Plan — VRTX3-T-0254: Add `/api/healthz-smoke-992401223-c`

## Objective

`GET /api/healthz-smoke-992401223-c` answers with `Content-Type: application/json` and the body
...
requirement **"Health probe C for variant 992401223"**. Each of this ticket's five acceptance
criteria is derived one-for-one from a scenario under that requirement, so a QA verdict traces
back to a named behaviour. `tasks.md` items 3.1–3.3 are tagged with this ticket key.
...
1. Create `routes/api/healthz-smoke-992401223-c.ts` with exactly this content →
...
2. Create `routes/api/healthz-smoke-992401223-c.test.ts` with exactly this content →
   **verify:** the single `it()` block asserts the returned object and there is no second case.
   ...
   import healthzC from "./healthz-smoke-992401223-c";

   describe("GET /api/healthz-smoke-992401223-c", () => {
     it("returns HTTP 200 with correct response body", async () => {
       const event = new H3Event(new Request("http://localhost/api/healthz-smoke-992401223-c"));
...
5. Produce a production build →
   **verify:** `.output/server/_routes/api/healthz_smoke_992401223_c.mjs` exists (Nitro converts
   dashes to underscores) and no `.test.ts` file was bundled.

## Interface contract — fixed, do not change

- **Path:** `/api/healthz-smoke-992401223-c`, produced solely by the filename
  `routes/api/healthz-smoke-992401223-c.ts`. There is no route table; a filename typo is a wrong URL
...
| File                                           | Action |
| ---------------------------------------------- | ------ |
| `routes/api/healthz-smoke-992401223-c.ts`      | create |
| `routes/api/healthz-smoke-992401223-c.test.ts` | create |
