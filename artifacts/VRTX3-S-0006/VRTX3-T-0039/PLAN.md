# TASK Plan — VRTX3-T-0035

**Title:** Implement `/api/healthz-smoke-913793173-b`
...
Add a simple GET endpoint at `/api/healthz-smoke-913793173-b` that returns `{ "ok": true, "variant": "913793173" }`. This endpoint is completely self-contained with no shared code, middleware, or database dependencies.

## File Ownership Map

### Create

- `routes/api/healthz-smoke-913793173-b.ts` — Route handler (Nitro H3 handler, 10 lines)
- `routes/api/healthz-smoke-913793173-b.test.ts` — Integration test (Vitest + H3Event, ~25 lines)
...
### 1. Route Handler: `routes/api/healthz-smoke-913793173-b.ts`
...
### 2. Integration Test: `routes/api/healthz-smoke-913793173-b.test.ts`
...
- [ ] Route file created at `routes/api/healthz-smoke-913793173-b.ts`
- [ ] Test file created at `routes/api/healthz-smoke-913793173-b.test.ts`
...
- [ ] Manual verification via `bun run dev` → `curl http://localhost:5000/api/healthz-smoke-913793173-b` returns correct JSON
...
1. Create `routes/api/healthz-smoke-913793173-b.ts` (5 min)
2. Create `routes/api/healthz-smoke-913793173-b.test.ts` (5 min)
...
- Existing pattern reference: `routes/api/healthz-smoke-302960562-b.ts` and `.test.ts`
