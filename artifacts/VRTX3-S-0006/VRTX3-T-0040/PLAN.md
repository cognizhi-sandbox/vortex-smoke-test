# TASK Plan — VRTX3-T-0036

**Title:** Implement `/api/healthz-smoke-913793173-c`
...
Add a simple GET endpoint at `/api/healthz-smoke-913793173-c` that returns `{ "ok": true, "variant": "913793173" }`. This endpoint is completely self-contained with no shared code, middleware, or database dependencies.

## File Ownership Map

### Create

- `routes/api/healthz-smoke-913793173-c.ts` — Route handler (Nitro H3 handler, 10 lines)
- `routes/api/healthz-smoke-913793173-c.test.ts` — Integration test (Vitest + H3Event, ~25 lines)
...
### 1. Route Handler: `routes/api/healthz-smoke-913793173-c.ts`
...
### 2. Integration Test: `routes/api/healthz-smoke-913793173-c.test.ts`

```typescript
import { H3Event } from "nitro/h3";
import { describe, expect, it } from "vitest";

import healthz from "./healthz-smoke-913793173-c";

describe("GET /api/healthz-smoke-913793173-c", () => {
  it("returns HTTP 200 with correct response body", async () => {
    const event = new H3Event(new Request("http://localhost/api/healthz-smoke-913793173-c"));
...
- [ ] Route file created at `routes/api/healthz-smoke-913793173-c.ts`
- [ ] Test file created at `routes/api/healthz-smoke-913793173-c.test.ts`
...
- [ ] Manual verification via `bun run dev` → `curl http://localhost:5000/api/healthz-smoke-913793173-c` returns correct JSON
...
1. Create `routes/api/healthz-smoke-913793173-c.ts` (5 min)
2. Create `routes/api/healthz-smoke-913793173-c.test.ts` (5 min)
...
- Existing pattern reference: `routes/api/healthz-smoke-302960562-c.ts` and `.test.ts`
