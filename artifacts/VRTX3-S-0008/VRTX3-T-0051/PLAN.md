# Plan — VRTX3-T-0051: `/api/healthz-smoke-bugfix3-221117839` does not serve JSON

## Objective

`GET /api/healthz-smoke-bugfix3-221117839` must return `200`, `Content-Type: application/json`,
body exactly `{"ok":true,"variant":"221117839"}`. Today no Nitro route module exists for that
...
The facilitator note on this ticket flagged the reported `404` as suspect and asked for
confirmation. **Confirmed — the facilitator is correct.** Reproduced on `bun run dev` (Vite,
port 5000) on 2026-08-08:

| Path                                             | Status | Content-Type                     | Body                                |
| ------------------------------------------------ | ------ | -------------------------------- | ----------------------------------- |
| `/api/healthz-smoke-bugfix3-221117839` (missing) | `200`  | `text/html; charset=utf-8`       | `<!doctype html>…` (SPA shell)      |
...
never `404`. No literal `404` was observed anywhere. The repro step in the ticket ("observe 404")
...
1. Create `routes/api/healthz-smoke-bugfix3-221117839.ts`, copying the shape of the existing
...
2. Create `routes/api/healthz-smoke-bugfix3-221117839.test.ts` following the H3Event integration
   ...
   `toEqual({ ok: true, variant: "221117839" })`.
...
- `routes/api/healthz-smoke-bugfix3-221117839.ts`
- `routes/api/healthz-smoke-bugfix3-221117839.test.ts`

No overlap with VRTX3-T-0049 or VRTX3-T-0050 — no `depends_on` required; all three may run fully
in parallel.

## Interface contracts (FIXED — do not change)

- Path: `/api/healthz-smoke-bugfix3-221117839` — note the `bugfix3` prefix
- Status: `200`
- `Content-Type`: `application/json` (served as `application/json;charset=UTF-8`)
- Body: `{ "ok": true, "variant": "221117839" }` — `ok` boolean `true`, `variant` **string**
  `"221117839"`, no other keys
...
- Method handling: handler is method-agnostic, matching every sibling. **Verified against the
  control**, closing the canvas's open "confirmed rather than assumed" item: `POST`, `PUT` and
  `DELETE` to `/api/healthz-smoke-bugfix3-605591646` each returned
  `200 application/json;charset=UTF-8` with the normal body — no 500, no method rejection. Do
  **not** add a method guard; that would diverge from the established pattern.
- No auth, no database, no middleware dependency, no code shared with any other endpoint

## Definition of Done

- [ ] `routes/api/healthz-smoke-bugfix3-221117839.ts` exists and default-exports a
      `defineHandler` returning `{ ok: true, variant: "221117839" }`.
- [ ] `routes/api/healthz-smoke-bugfix3-221117839.test.ts` exists, follows the H3Event pattern
...
- **Manual runtime check (the honest signal):** `bun run dev`, then
  ...
  Expect `{"ok":true,"variant":"221117839"}` with `200 application/json;charset=UTF-8`. Seeing
  `text/html` means the route is still not registered — the filename is wrong or the file is
  missing.
...
- **Optional production-build check:** `bun run build` emits one module per route under
  ...
  `.output/server/_routes/api/healthz_smoke_bugfix3_221117839.mjs`.
