# Plan — VRTX3-T-0049: `/api/healthz-smoke-bugfix-739648350` does not serve JSON

## Objective

`GET /api/healthz-smoke-bugfix-739648350` must return `200`, `Content-Type: application/json`,
body exactly `{"ok":true,"variant":"739648350"}`. Today no Nitro route module exists for that
...
| ------------------------------------------------ | ------ | -------------------------------- | ----------------------------------- |
| `/api/healthz-smoke-bugfix-739648350` (missing)  | `200`  | `text/html; charset=utf-8`       | `<!doctype html>…` (SPA shell)      |
...
1. Create `routes/api/healthz-smoke-bugfix-739648350.ts`, copying the shape of the existing
...
2. Create `routes/api/healthz-smoke-bugfix-739648350.test.ts` following the H3Event integration
   ...
   `toEqual({ ok: true, variant: "739648350" })`.
...
- `routes/api/healthz-smoke-bugfix-739648350.ts`
- `routes/api/healthz-smoke-bugfix-739648350.test.ts`

No overlap with VRTX3-T-0050 or VRTX3-T-0051 — no `depends_on` required; all three may run fully
in parallel.

## Interface contracts (FIXED — do not change)

- Path: `/api/healthz-smoke-bugfix-739648350`
- Status: `200`
- `Content-Type`: `application/json` (served as `application/json;charset=UTF-8`)
- Body: `{ "ok": true, "variant": "739648350" }` — `ok` boolean `true`, `variant` **string**
  `"739648350"`, no other keys
...
- [ ] `routes/api/healthz-smoke-bugfix-739648350.ts` exists and default-exports a `defineHandler`
      returning `{ ok: true, variant: "739648350" }`.
- [ ] `routes/api/healthz-smoke-bugfix-739648350.test.ts` exists, follows the H3Event pattern
...
- **Manual runtime check (the honest signal):** `bun run dev`, then
  ...
  Expect `{"ok":true,"variant":"739648350"}` with `200 application/json;charset=UTF-8`. Seeing
...
- **Optional production-build check:** `bun run build` emits one module per route under
  ...
  `.output/server/_routes/api/healthz_smoke_bugfix_739648350.mjs`.
