# Plan — VRTX3-T-0055: `/api/healthz-smoke-bugfix-755467473` returns the SPA shell instead of `{ok,variant}`
...
registration. `grep -rl 755467473 .` over the repo returns **no match** — the handler file was
...
| ----------------------------------------------- | ------ | -------------------------------- | ----------------------------------- |
| `/api/healthz-smoke-bugfix-755467473` (today)   | `200`  | `text/html; charset=utf-8`       | `<!doctype html>…`                  |
| `/api/healthz-smoke-bugfix-739648350` (control) | `200`  | `application/json;charset=UTF-8` | `{"ok":true,"variant":"739648350"}` |
...
`GET /api/healthz-smoke-bugfix-755467473` serves `200` with `Content-Type: application/json`
and the exact body `{"ok":true,"variant":"755467473"}`, backed by a co-located H3Event
integration test — matching the 44 existing `healthz-smoke-*` siblings byte-for-byte in shape.

## Steps

1. Create `routes/api/healthz-smoke-bugfix-755467473.ts` by copying
   `routes/api/healthz-smoke-bugfix-739648350.ts` and changing only the variant digits:
...
2. Create `routes/api/healthz-smoke-bugfix-755467473.test.ts` by copying
   `routes/api/healthz-smoke-bugfix-739648350.test.ts`, updating the import path, the
...
| `routes/api/healthz-smoke-bugfix-755467473.test.ts` | **new** |
...
No overlap with VRTX3-T-0056 or VRTX3-T-0057 — all three run in parallel, no `depends_on`.

## Interface contracts (FIXED — do not change)

- **Route path**: `/api/healthz-smoke-bugfix-755467473`, derived solely from the filename.
- **Response body**: exactly `{ ok: true, variant: "755467473" }` — two keys, `variant` is a
  **string** of digits, no extra fields.
...
1. `routes/api/healthz-smoke-bugfix-755467473.ts` exists and default-exports a `defineHandler`
   from `"nitro/h3"` returning exactly `{ ok: true, variant: "755467473" }`, structurally
   identical to `routes/api/healthz-smoke-bugfix-739648350.ts`.
2. On a running dev server, `GET /api/healthz-smoke-bugfix-755467473` responds with
   `Content-Type` matching `application/json` and a body deep-equal to
   `{"ok":true,"variant":"755467473"}`. All three of status, content type and body are checked;
   a status-code-only assertion is explicitly insufficient here.
3. `routes/api/healthz-smoke-bugfix-755467473.test.ts` exists, follows the H3Event integration
   pattern of `routes/api/healthz-smoke-bugfix-739648350.test.ts` (construct
   `new H3Event(new Request(...))`, await the default export), and asserts the returned object
   deep-equals `{ ok: true, variant: "755467473" }`.
...
| --------------------------------------------- | --------------------------------------------------- | ----------------------------------------------------------------------- |
| `returns HTTP 200 with correct response body` | `routes/api/healthz-smoke-bugfix-755467473.test.ts` | `await handler(event)` deep-equals `{ ok: true, variant: "755467473" }` |
