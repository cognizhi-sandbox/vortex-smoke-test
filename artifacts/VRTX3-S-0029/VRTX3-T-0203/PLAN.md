...
# Plan — VRTX3-T-0203: `/api/healthz-smoke-bugfix-ha-971401638` returns the SPA shell, not its probe body

## Objective

`GET /api/healthz-smoke-bugfix-ha-971401638` answers HTTP 200 with `Content-Type: application/json`
and a body deep-equal to `{ "ok": true, "variant": "971401638" }`, served by a new self-contained
Nitro route with a colocated test. Two new files; no existing source file changes.

## Root cause

**The handler was never written.** `routes/api/healthz-smoke-bugfix-ha-971401638.ts` does not exist,
and Nitro registers routes by filename alone — `nitro({ serverDir: "./", ignore: ["**/*.test.ts"] })`
in `vite.config.ts:29`, with no route table (`ARCHITECTURE.md` § Routing: "The filename **is** the
URL contract"). No file, no route. This is a missing-file gap, not a regression: nothing was broken
by a change, no middleware or route ordering is involved, and no existing route behaves differently.

A repo-wide grep for `971401638` (excluding `node_modules`/`.git`) returned zero matches, confirming
...
```
/healthz-smoke-bugfix-ha-971401638       200 text/html; charset=utf-8       949B  (SPA index.html shell)
/api/healthz-smoke-bugfix-ha-971401638   200 text/html; charset=utf-8       949B  (SPA index.html shell)
...
1. Copy `routes/api/healthz-smoke-528856326-a.ts` to
   `routes/api/healthz-smoke-bugfix-ha-971401638.ts`, changing only the variant string to
   `"971401638"`. Change nothing else — no `event` parameter, no method guard, no import beyond
   `nitro/h3`.
2. Copy `routes/api/healthz-smoke-528856326-a.test.ts` to
   `routes/api/healthz-smoke-bugfix-ha-971401638.test.ts`, updating the import path, the `describe`
...
- `routes/api/healthz-smoke-bugfix-ha-971401638.ts`
- `routes/api/healthz-smoke-bugfix-ha-971401638.test.ts`

Modifies nothing. **Does not intersect VRTX3-T-0204** — no `depends_on` in either direction; the two
...
- Route path: `/api/healthz-smoke-bugfix-ha-971401638` — the filename is the URL; a typo is a dead
  route with no other symptom.
- `variant` is the **string** `"971401638"`, not a number.
...
- DoD-1: `routes/api/healthz-smoke-bugfix-ha-971401638.ts` exists and default-exports a single
  `defineHandler` from `nitro/h3` returning the literal `{ ok: true, variant: "971401638" }`.
- DoD-2: A live `GET /api/healthz-smoke-bugfix-ha-971401638` against a running dev server returns
  HTTP 200 with `Content-Type: application/json` and body exactly
  `{"ok":true,"variant":"971401638"}` — asserted on body and `Content-Type`, not status code alone.
...
- DoD-4: `routes/api/healthz-smoke-bugfix-ha-971401638.test.ts` constructs an `H3Event` and asserts
  `toEqual({ ok: true, variant: "971401638" })` as its single case, with no timing assertion.
- DoD-5: No file outside the two new ones is added, modified or deleted, and no dependency is added.

## Test plan

- **API integration**, `routes/api/healthz-smoke-bugfix-ha-971401638.test.ts` — one case, collected
  by the Vitest `server` project (`environment: "node"`) with no configuration change. Expected: the
  handler's returned object deep-equals `{ ok: true, variant: "971401638" }`. Covers DoD-1, DoD-4.
