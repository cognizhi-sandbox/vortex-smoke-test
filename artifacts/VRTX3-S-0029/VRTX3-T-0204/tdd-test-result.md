...
# TDD result — VRTX3-T-0204

## Test cases

| Test                                                                                                  | Covers       | Intent                                                           |
| ----------------------------------------------------------------------------------------------------- | ------------ | ---------------------------------------------------------------- |
| `routes/api/healthz-smoke-bugfix-ha2-649579386.test.ts › returns HTTP 200 with correct response body` | DoD-1, DoD-4 | handler returns the literal `{ ok: true, variant: "649579386" }` |

## Red run

`bun --bun vitest run routes/api/healthz-smoke-bugfix-ha2-649579386.test.ts` (test file committed
before the handler existed):

```
 FAIL  |server| routes/api/healthz-smoke-bugfix-ha2-649579386.test.ts [ routes/api/healthz-smoke-bugfix-ha2-649579386.test.ts ]
Error: Cannot find module './healthz-smoke-bugfix-ha2-649579386' imported from /workspace/repo/routes/api/healthz-smoke-bugfix-ha2-649579386.test.ts
