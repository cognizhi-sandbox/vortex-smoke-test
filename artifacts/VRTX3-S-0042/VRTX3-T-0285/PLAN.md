# PLAN — VRTX3-T-0285 · Add GET /api/healthz-smoke-613529736-b
...
Serve `GET /api/healthz-smoke-613529736-b` with a fixed `{ ok: true, variant: "613529736" }` JSON
...
2. Copy `routes/api/healthz-smoke-528856326-a.ts` to `routes/api/healthz-smoke-613529736-b.ts` and
...
3. Copy `routes/api/healthz-smoke-528856326-a.test.ts` to
   `routes/api/healthz-smoke-613529736-b.test.ts`, updating the import path, the `describe` title
...
6. Verify the production build emits `.output/server/_routes/api/healthz_smoke_613529736_b.mjs`.

## File / module ownership

Creates exactly two files, modifies none:

- `routes/api/healthz-smoke-613529736-b.ts`
- `routes/api/healthz-smoke-613529736-b.test.ts`

Read-only: `routes/api/healthz-smoke-528856326-a.ts` and its `.test.ts` sibling, `vite.config.ts`,
`vitest.config.ts`. The `-a` and `-c` probes belong to VRTX3-T-0284 and VRTX3-T-0286 — do not
create, edit or import them. No dependency edge exists between the three tickets; they may merge in
any order.

## Definition of Done

AC-1 through AC-5 on VRTX3-T-0285 are met, and the diff adds exactly the two files listed above and
