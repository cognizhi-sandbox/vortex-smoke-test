# Plan — VRTX3-T-0066: Implement GET /api/healthz-smoke-46132092-c
...
`GET /api/healthz-smoke-46132092-c` on the running app answers with `Content-Type:
...
`routes/api/`, and it shares no code with its two peer endpoints (`-a`, `-b`) being built in
parallel.

## Steps

1. **Re-check the name is still free.** `grep -rl "46132092" routes/` — expect no hits for the
   `-c` files. (It was clean at planning time on 2026-08-09.)
2. **Create `routes/api/healthz-smoke-46132092-c.ts`**, copying
...
3. **Create `routes/api/healthz-smoke-46132092-c.test.ts`**, copying
   ...
   import healthz from "./healthz-smoke-46132092-c";

   describe("GET /api/healthz-smoke-46132092-c", () => {
     it("returns HTTP 200 with correct response body", async () => {
       const event = new H3Event(new Request("http://localhost/api/healthz-smoke-46132092-c"));
...
5. **Confirm the route compiled into the production server**: the build emits
   `.output/server/_routes/api/healthz_smoke_46132092_c.mjs` (dashes become underscores).
...
| File                                          | Purpose                              |
| --------------------------------------------- | ------------------------------------ |
| `routes/api/healthz-smoke-46132092-c.ts`      | The handler (8 lines)                |
| `routes/api/healthz-smoke-46132092-c.test.ts` | H3Event integration test (~25 lines) |

**Modify:** none.

**Do not touch** — owned by a peer TASK or frozen for this sprint: any `*-46132092-a*` or
`*-46132092-b*` file (VRTX3-T-0064 / VRTX3-T-0065), the 45 existing `routes/api/` handlers,
...
- The module **default-exports** the result of `defineHandler` imported from `"nitro/h3"`, so
  the test can `import healthz from "./healthz-smoke-46132092-c"` and call it with an
  `H3Event`.
- Path is exactly `/api/healthz-smoke-46132092-c`, derived from the filename — there is no
...
| Production build output                       | Build artefact inspection | `.../_routes/api/healthz_smoke_46132092_c.mjs` exists |
