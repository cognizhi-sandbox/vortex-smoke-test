...
# Plan — VRTX3-T-0260: Add `/api/healthz-smoke-812788042-a`

## Objective

`GET /api/healthz-smoke-812788042-a` answers with `Content-Type: application/json` and the body
...
requirement **"Health probe A for variant 812788042"**. Each of this ticket's five acceptance
...
1. Create `routes/api/healthz-smoke-812788042-a.ts` with exactly this content →
...
2. Create `routes/api/healthz-smoke-812788042-a.test.ts` with exactly this content →
...
5. Produce a production build →
   **verify:** `.output/server/_routes/api/healthz_smoke_812788042_a.mjs` exists (Nitro converts
   dashes to underscores) and no `.test.ts` file was bundled.

## Interface contract — fixed, do not change

- **Path:** `/api/healthz-smoke-812788042-a`, produced solely by the filename
  `routes/api/healthz-smoke-812788042-a.ts`. There is no route table; a filename typo is a wrong URL
...
| File                                           | Action |
| ---------------------------------------------- | ------ |
| `routes/api/healthz-smoke-812788042-a.ts`      | create |
| `routes/api/healthz-smoke-812788042-a.test.ts` | create |
