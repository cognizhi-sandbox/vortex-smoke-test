...
# Plan — VRTX3-T-0262: Add `/api/healthz-smoke-812788042-c`

## Objective

`GET /api/healthz-smoke-812788042-c` answers with `Content-Type: application/json` and the body
...
requirement **"Health probe C for variant 812788042"**. Each of this ticket's five acceptance
...
1. Create `routes/api/healthz-smoke-812788042-c.ts` with exactly this content →
...
2. Create `routes/api/healthz-smoke-812788042-c.test.ts` with exactly this content →
   **verify:** the single `it()` block asserts the returned object and there is no second case.
   ...
   import healthzC from "./healthz-smoke-812788042-c";

   describe("GET /api/healthz-smoke-812788042-c", () => {
     it("returns HTTP 200 with correct response body", async () => {
       const event = new H3Event(new Request("http://localhost/api/healthz-smoke-812788042-c"));
...
5. Produce a production build →
   **verify:** `.output/server/_routes/api/healthz_smoke_812788042_c.mjs` exists (Nitro converts
   dashes to underscores) and no `.test.ts` file was bundled.

## Interface contract — fixed, do not change

- **Path:** `/api/healthz-smoke-812788042-c`, produced solely by the filename
  `routes/api/healthz-smoke-812788042-c.ts`. There is no route table; a filename typo is a wrong URL
...
| File                                           | Action |
| ---------------------------------------------- | ------ |
| `routes/api/healthz-smoke-812788042-c.ts`      | create |
| `routes/api/healthz-smoke-812788042-c.test.ts` | create |
