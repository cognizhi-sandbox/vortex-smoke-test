...
# Plan — VRTX3-T-0167: `/api/healthz-smoke-bugfix-27681476` returns the SPA shell, not its probe body

## Objective

`GET /api/healthz-smoke-bugfix-27681476` answers HTTP 200 with `Content-Type: application/json` and
a body deep-equal to `{ "ok": true, "variant": "27681476" }`, served by a new self-contained Nitro
route with a colocated test. Two new files; no existing source file changes.

## Root cause

**The handler was never written.** `routes/api/healthz-smoke-bugfix-27681476.ts` does not exist, and
...
A repo-wide grep for `27681476` (excluding `node_modules`/`.git`) returned zero matches, confirming
...
```
/api/healthz-smoke-bugfix-27681476   200 text/html; charset=utf-8       949b  (SPA index.html shell)
...
1. Copy `routes/api/healthz-smoke-528856326-a.ts` to
   `routes/api/healthz-smoke-bugfix-27681476.ts`, changing only the variant string to `"27681476"`.
   Change nothing else — no `event` parameter, no method guard, no import beyond `nitro/h3`.
2. Copy `routes/api/healthz-smoke-528856326-a.test.ts` to
   `routes/api/healthz-smoke-bugfix-27681476.test.ts`, updating the import path, the `describe`
...
- `routes/api/healthz-smoke-bugfix-27681476.ts`
- `routes/api/healthz-smoke-bugfix-27681476.test.ts`

Modifies nothing. **Does not intersect VRTX3-T-0168 or VRTX3-T-0169** — no `depends_on` in either
...
- Route path: `/api/healthz-smoke-bugfix-27681476` — the filename is the URL; a typo is a dead route
  with no other symptom.
- `variant` is the **string** `"27681476"`, not a number.
...
- DoD-1: `routes/api/healthz-smoke-bugfix-27681476.ts` exists and default-exports a single
  `defineHandler` from `nitro/h3` returning the literal `{ ok: true, variant: "27681476" }`.
- DoD-2: A live `GET /api/healthz-smoke-bugfix-27681476` against a running dev server returns HTTP
  200 with `Content-Type: application/json` and body exactly `{"ok":true,"variant":"27681476"}` —
...
- DoD-4: `routes/api/healthz-smoke-bugfix-27681476.test.ts` constructs an `H3Event` and asserts
  `toEqual({ ok: true, variant: "27681476" })` as its single case, with no timing assertion.
- DoD-5: No file outside the two new ones is added, modified or deleted, and no dependency is added.

## Test plan

- **API integration**, `routes/api/healthz-smoke-bugfix-27681476.test.ts` — one case, collected by
  the Vitest `server` project (`environment: "node"`) with no configuration change. Expected: the
  handler's returned object deep-equals `{ ok: true, variant: "27681476" }`. Covers DoD-1, DoD-4.
