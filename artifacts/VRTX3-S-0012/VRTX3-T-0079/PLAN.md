# VRTX3-T-0079 — Fix plan

**Sprint:** VRTX3-S-0012 · **Type:** DEFECT · **Route:** `/api/healthz-smoke-bugfix3-196651982` · **Variant:** `"196651982"`
...
```
GET /api/healthz-smoke-bugfix3-196651982
...
```bash
$ grep -rn "196651982" . --exclude-dir=node_modules --exclude-dir=.git
...
| Filename typo under a near-miss name       | Repo-wide grep for `196651982` returns zero matches                                                                 |
...
1. `routes/api/healthz-smoke-bugfix3-196651982.ts` — default-export a single `defineHandler` imported from `nitro/h3`,
   taking no parameters, returning the literal `{ ok: true, variant: "196651982" }`.
2. `routes/api/healthz-smoke-bugfix3-196651982.test.ts` — mirror the sibling spec: construct an `H3Event` from a
   `Request` for the route's URL, call the handler, assert the resolved value equals
   `{ ok: true, variant: "196651982" }`, and assert it resolves in under 100ms.

**Fixed interface contract — do not deviate:**

```ts
// routes/api/healthz-smoke-bugfix3-196651982.ts
...
- `routes/api/healthz-smoke-bugfix3-196651982.ts` exists and default-exports a single `defineHandler` from `nitro/h3`
  taking no parameters, matching the contract above exactly.
- `GET /api/healthz-smoke-bugfix3-196651982` on a running server responds with `Content-Type: application/json` and the
  body exactly `{"ok":true,"variant":"196651982"}` — no extra keys, `variant` a string not a number.
- `routes/api/healthz-smoke-bugfix3-196651982.test.ts` exists, constructs an `H3Event`, and its two assertions pass: the
  handler resolves to `{ ok: true, variant: "196651982" }`, and it resolves in under 100ms.
- The production build emits `.output/server/_routes/api/healthz_smoke_bugfix3_196651982.mjs`, and no
...
| File                                                 | Action |
| ---------------------------------------------------- | ------ |
| `routes/api/healthz-smoke-bugfix3-196651982.ts`      | create |
| `routes/api/healthz-smoke-bugfix3-196651982.test.ts` | create |
...
for `196651982` returns zero matches, so the new filename cannot shadow or be shadowed by an existing
