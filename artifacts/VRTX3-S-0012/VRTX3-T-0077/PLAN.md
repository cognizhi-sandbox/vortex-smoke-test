# VRTX3-T-0077 — Fix plan

**Sprint:** VRTX3-S-0012 · **Type:** DEFECT · **Route:** `/api/healthz-smoke-bugfix-6202295` · **Variant:** `"6202295"`
...
```
GET /api/healthz-smoke-bugfix-6202295
...
```bash
$ grep -rn "6202295" . --exclude-dir=node_modules --exclude-dir=.git
...
| Filename typo under a near-miss name       | Repo-wide grep for `6202295` returns zero matches                                                                   |
...
1. `routes/api/healthz-smoke-bugfix-6202295.ts` — default-export a single `defineHandler` imported from `nitro/h3`,
   taking no parameters, returning the literal `{ ok: true, variant: "6202295" }`.
2. `routes/api/healthz-smoke-bugfix-6202295.test.ts` — mirror the sibling spec: construct an `H3Event` from a
   `Request` for the route's URL, call the handler, assert the resolved value equals
   `{ ok: true, variant: "6202295" }`, and assert it resolves in under 100ms.

**Fixed interface contract — do not deviate:**

```ts
// routes/api/healthz-smoke-bugfix-6202295.ts
...
- `routes/api/healthz-smoke-bugfix-6202295.ts` exists and default-exports a single `defineHandler` from `nitro/h3`
  taking no parameters, matching the contract above exactly.
- `GET /api/healthz-smoke-bugfix-6202295` on a running server responds with `Content-Type: application/json` and the
  body exactly `{"ok":true,"variant":"6202295"}` — no extra keys, `variant` a string not a number.
- `routes/api/healthz-smoke-bugfix-6202295.test.ts` exists, constructs an `H3Event`, and its two assertions pass: the
  handler resolves to `{ ok: true, variant: "6202295" }`, and it resolves in under 100ms.
- The production build emits `.output/server/_routes/api/healthz_smoke_bugfix_6202295.mjs`, and no
...
| File                                              | Action |
| ------------------------------------------------- | ------ |
| `routes/api/healthz-smoke-bugfix-6202295.ts`      | create |
| `routes/api/healthz-smoke-bugfix-6202295.test.ts` | create |
...
for `6202295` returns zero matches, so the new filename cannot shadow or be shadowed by an existing
