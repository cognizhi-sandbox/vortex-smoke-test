...
# Plan — VRTX3-T-0206: `/api/healthz-smoke-bugfix-ha-853006542` returns the SPA shell, not its probe body

## Objective

`GET /api/healthz-smoke-bugfix-ha-853006542` answers HTTP 200 with `Content-Type: application/json`
and a body deep-equal to `{ "ok": true, "variant": "853006542" }`, served by a new self-contained
Nitro route with a colocated test. Two new files; no existing source file changes.

## Root cause

**The handler was never written.** `routes/api/healthz-smoke-bugfix-ha-853006542.ts` does not exist,
...
A repo-wide grep for `853006542` (excluding `node_modules`/`.git`) returned zero matches, confirming
...
```
/api/healthz-smoke-bugfix-ha-853006542   200 text/html; charset=utf-8       949b  (SPA index.html shell)
/healthz-smoke-bugfix-ha-853006542       200 text/html; charset=utf-8       949b  (SPA index.html shell)
...
defect. The ticket writes `/healthz-smoke-bugfix-ha-853006542`; every one of the 95 existing probes
...
`/api/healthz-smoke-bugfix-ha-853006542`.

## Steps

1. Copy `routes/api/healthz-smoke-528856326-a.ts` to
   `routes/api/healthz-smoke-bugfix-ha-853006542.ts`, changing only the variant string to
   `"853006542"`. Change nothing else — no `event` parameter, no method guard, no import beyond
   `nitro/h3`.
2. Copy `routes/api/healthz-smoke-528856326-a.test.ts` to
   `routes/api/healthz-smoke-bugfix-ha-853006542.test.ts`, updating the import path, the `describe`
...
- `routes/api/healthz-smoke-bugfix-ha-853006542.ts`
- `routes/api/healthz-smoke-bugfix-ha-853006542.test.ts`

Modifies nothing. **Does not intersect VRTX3-T-0207** — no `depends_on` in either direction; the two
...
- Route path: `/api/healthz-smoke-bugfix-ha-853006542` — the filename is the URL; a typo is a dead
  route with no other symptom. Note the `-ha-` infix: no existing probe carries it, so there is no
  neighbouring filename to pattern-match against.
- `variant` is the **string** `"853006542"`, not a number.
...
- DoD-1: `routes/api/healthz-smoke-bugfix-ha-853006542.ts` exists and default-exports a single
  `defineHandler` from `nitro/h3` returning the literal `{ ok: true, variant: "853006542" }`.
- DoD-2: A live `GET /api/healthz-smoke-bugfix-ha-853006542` against a running dev server returns
  HTTP 200 with `Content-Type: application/json` and body exactly
  `{"ok":true,"variant":"853006542"}` — asserted on body and `Content-Type`, not status code alone.
...
- DoD-4: `routes/api/healthz-smoke-bugfix-ha-853006542.test.ts` constructs an `H3Event` and asserts
  `toEqual({ ok: true, variant: "853006542" })` as its single case, with no timing assertion.
- DoD-5: No file outside the two new ones is added, modified or deleted, and no dependency is added.

## Test plan

- **API integration**, `routes/api/healthz-smoke-bugfix-ha-853006542.test.ts` — one case, collected
  by the Vitest `server` project (`environment: "node"`) with no configuration change. Expected: the
  handler's returned object deep-equals `{ ok: true, variant: "853006542" }`. Covers DoD-1, DoD-4.
