# VRTX3-T-0013: Fix /api/healthz-smoke-bugfix-26031336 → 404

**Defect ID:** smoke-bugfix-17856540658772  
**Endpoint:** GET `/api/healthz-smoke-bugfix-26031336`  
**Variant ID:** 26031336
...
The endpoint `/api/healthz-smoke-bugfix-26031336` returns HTTP 404 Not Found. Expected behavior: HTTP 200 with response body `{"ok":true,"variant":"26031336"}`.
...
The route file `routes/api/healthz-smoke-bugfix-26031336.ts` **does not exist**. Nitro's file-based router has no handler for this endpoint, so all requests return 404.
...
### File 1: `routes/api/healthz-smoke-bugfix-26031336.ts`
...
### File 2: `routes/api/healthz-smoke-bugfix-26031336.test.ts`

```typescript
import { H3Event } from "nitro/h3";
import { describe, expect, it } from "vitest";

import healthz from "./healthz-smoke-bugfix-26031336";

describe("GET /api/healthz-smoke-bugfix-26031336", () => {
  it("returns HTTP 200 with correct response body", async () => {
    const event = new H3Event(new Request("http://localhost/api/healthz-smoke-bugfix-26031336"));

    const result = await healthz(event);

    expect(result).toEqual({ ok: true, variant: "26031336" });
  });
...
- [ ] `routes/api/healthz-smoke-bugfix-26031336.ts` created with specified handler
- [ ] `routes/api/healthz-smoke-bugfix-26031336.test.ts` created with both test cases passing
...
curl http://localhost:5000/api/healthz-smoke-bugfix-26031336
# Should return: {"ok":true,"variant":"26031336"}
```
