# VRTX3-T-0125 — Fix plan

**Sprint:** VRTX3-S-0018 · **Type:** DEFECT · **Route:** `/api/healthz-smoke-bugfix3-850084489` · **Variant:** `"850084489"`

Sprint-level index and shared notes: [`../SPRINT-PLAN.md`](../SPRINT-PLAN.md).
Source idea: VRTX3-I-0027 (canvas doc v11).
...
**The ticket's reported symptom "404" is incorrect** — and this idea canvas says so itself, having
measured the same `200 text/html; charset=utf-8`, 949-byte SPA shell. This planning pass re-measured
independently rather than citing it, and the two agree. An unmatched `/api/*` path is answered by
the SPA `index.html` fallback with **HTTP 200**, in dev and in the production build alike —
`nginx.conf:28` proxies `location /api/` straight to Nitro and does not set `proxy_intercept_errors`,
so it does not change this either. The defect is real; only its stated status code is wrong.
Consequence for this ticket: a `404 → 200` assertion proves nothing, because the path already
returns 200. **Verify on the response body and `Content-Type`.**
...
```bash
$ grep -rl "850084489" . --exclude-dir=node_modules --exclude-dir=.git
...
| Filename typo under a near-miss name       | Repo-wide grep for `850084489` returns zero matches                                                                 |
...
1. `routes/api/healthz-smoke-bugfix3-850084489.ts` — default-export a single `defineHandler`
   imported from `nitro/h3`, taking no parameters, returning the literal
   `{ ok: true, variant: "850084489" }`.
2. `routes/api/healthz-smoke-bugfix3-850084489.test.ts` — mirror the sibling spec: construct an
   `H3Event` from a `Request` for the route's URL, call the handler, and assert the resolved value
   equals `{ ok: true, variant: "850084489" }`. **One assertion only.**
...
- **Copy the `528856326` pair, not an older probe.** Tests written before VRTX3-S-0011 carry a
  ...
  (`AGENT.md` § Health Probe Routes). The canvas for this idea names the same `528856326` pair, so
  there is no conflict to resolve this time.
...
- Root docs (`AGENT.md` / `PRODUCT.md` / `ARCHITECTURE.md` / `DESIGN.md`) are **out of scope**; they
  were brought to target state on the planning ticket. The canvas's regression-risk table mentions
  bumping the probe count — that is planning's job and is already done. Do not modify them.

## 4. Definition of Done

- `routes/api/healthz-smoke-bugfix3-850084489.ts` exists and default-exports a single
  `defineHandler` from `nitro/h3` taking no parameters, matching the contract above exactly.
- `GET /api/healthz-smoke-bugfix3-850084489` on a freshly started server responds with
  `Content-Type: application/json` and the body exactly `{"ok":true,"variant":"850084489"}` — no
...
- `routes/api/healthz-smoke-bugfix3-850084489.test.ts` exists, constructs an `H3Event` for the
  route's URL, and its single assertion — the handler resolves to
  `{ ok: true, variant: "850084489" }` — passes. It carries no elapsed-time assertion.
- The production build emits `.output/server/_routes/api/healthz_smoke_bugfix3_850084489.mjs`, and
...
| File                                                 | Action |
| ---------------------------------------------------- | ------ |
| `routes/api/healthz-smoke-bugfix3-850084489.ts`      | create |
| `routes/api/healthz-smoke-bugfix3-850084489.test.ts` | create |
...
for `850084489` returns zero matches, so the new filename cannot shadow or be shadowed by an
