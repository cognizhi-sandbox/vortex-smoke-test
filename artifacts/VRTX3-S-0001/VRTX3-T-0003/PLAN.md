# VRTX3-T-0003 Implementation Plan

**Defect**: GET `/api/healthz-smoke-bugfix3-429794134` returns 404, should return 200 + `{"ok":true,"variant":"429794134"}`

**Root Cause**: Route file `routes/api/healthz-smoke-bugfix3-429794134.ts` missing from the Nitro server.
...
1. ✅ Route file created at `routes/api/healthz-smoke-bugfix3-429794134.ts`
2. ✅ Handler returns `{ ok: true, variant: "429794134" }` for all requests
3. ✅ Nitro server auto-registers route to `/api/healthz-smoke-bugfix3-429794134` via file-based routing
4. ✅ Test file created at `routes/api/healthz-smoke-bugfix3-429794134.test.ts` with:
   - Correct JSON response body assertion
   - Performance assertion (< 100ms)
5. ✅ Test passes: `bun run test -- healthz-smoke-bugfix3-429794134.test.ts`
...
File: `routes/api/healthz-smoke-bugfix3-429794134.test.ts`

```typescript
import { H3Event } from "nitro/h3";
import { describe, expect, it } from "vitest";

import healthz from "./healthz-smoke-bugfix3-429794134";

describe("GET /api/healthz-smoke-bugfix3-429794134", () => {
  it("returns HTTP 200 with correct response body", async () => {
    const event = new H3Event(new Request("http://localhost/api/healthz-smoke-bugfix3-429794134"));

    const result = await healthz(event);

    expect(result).toEqual({ ok: true, variant: "429794134" });
  });
...
- **Created**: `routes/api/healthz-smoke-bugfix3-429794134.ts`
- **Created**: `routes/api/healthz-smoke-bugfix3-429794134.test.ts`

## Commit Message

```
Add /api/healthz-smoke-bugfix3-429794134 health check endpoint

- Adds missing route handler returning {ok: true, variant: "429794134"}
