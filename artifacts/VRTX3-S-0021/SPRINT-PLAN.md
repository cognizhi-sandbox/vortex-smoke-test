# Sprint Plan — VRTX3-S-0021

**Title:** Three Independent Health Check Endpoints (568557289)
**Idea:** VRTX3-I-0030 — `[smoke-178647172883290] 3 independent endpoints (568557289)` (enhancement)
**Planning ticket:** VRTX3-T-0143
**Created:** 2026-08-11
...
Ship three standalone Nitro health probes — `GET /api/healthz-smoke-568557289-a`, `-b` and `-c` — each answering with `Content-Type: application/json` and a body deep-equal to `{ "ok": true, "variant": "568557289" }`.
...
Read this sprint: `routes/api/` (150 `.ts` files), `vite.config.ts`, `vitest.config.ts`, `.github/workflows/ci.yml`, and the four root docs. Everything below is measured against this working tree, not carried forward from a prior plan.

**The pattern already exists 74 times over.** Measured: `ls routes/api/healthz-smoke-*.ts | grep -v '\.test\.ts$' | wc -l` → **74** handlers, plus 74 colocated tests (148 of the 150 files under `routes/api/`, the remainder being `hello.ts`/`hello.test.ts` and the `users/` dynamic-route example). `routes/api/healthz-smoke-528856326-a.ts` is the whole shape, verbatim:
...
**Nothing named `568557289` exists yet.** A repo-wide `grep -rl 568557289` (excluding `node_modules` and `.git`) returned zero matches — the change is purely additive: six new files, zero modified source files. These are never-written files, not typo'd filenames.

**The SPA-fallback baseline was re-measured on a live dev server this sprint, not cited.** Against `bun run dev`, which bound port **5001** in this container (the banner printed `Port 5000 is in use, trying another one...` — read the banner, never assume the port):

| Path                                | Status | `Content-Type`                   | Size  | Body                                |
| ----------------------------------- | ------ | -------------------------------- | ----- | ----------------------------------- |
| `/api/healthz-smoke-568557289-a`    | `200`  | `text/html; charset=utf-8`       | 949 B | `<!doctype html>…` (the SPA shell)  |
| `/api/healthz-smoke-568557289-b`    | `200`  | `text/html; charset=utf-8`       | 949 B | `<!doctype html>…`                  |
| `/api/healthz-smoke-568557289-c`    | `200`  | `text/html; charset=utf-8`       | 949 B | `<!doctype html>…`                  |
| `/api/healthz-smoke-528856326-a` ✅ | `200`  | `application/json;charset=UTF-8` | 33 B  | `{"ok":true,"variant":"528856326"}` |

**Thirteenth consecutive sprint to confirm it, and the fourth on an enhancement with no incoming `404` claim to debunk.** A missing `/api/*` path is **indistinguishable from a working one by status code** — so every verification in this sprint asserts on the body and `Content-Type`, never on a status code or a `404 → 200` transition. The idea canvas states the behaviour correctly in its own risk register rather than asserting a `404`, so there is nothing to correct upstream this time. The measurement was still taken, because a canvas getting this right is evidence about the canvas, not about the working tree — only a live request tells you whether the file exists today. See [AGENT.md § Gotchas](../../AGENT.md#gotchas).

**The idea names the correct test copy-source — the fourth time in a row.** The canvas points at `routes/api/healthz-smoke-528856326-a.test.ts` and reproduces the rule that the pointer outranks any file an idea names. Measured this sprint: **47 of the 74** probe tests still carry a second `expect(elapsed).toBeLessThan(100)` case — machine-dependent, a known CI-flake source, deliberately dropped in VRTX3-S-0011. So there is **no substitution to make this sprint**; each PLAN.md still pins the source file by name so the canonical shape cannot drift back.
...
**Root doc counts were consistent at 74** across `AGENT.md` § Health Probe Routes, `ARCHITECTURE.md` § Routing and `PRODUCT.md` § Features, each verified against the filesystem this sprint before being bumped. The stale-count drift that VRTX3-S-0015 corrected has not recurred.

---

## Target state of the root docs

Brought to target state on the planning ticket branch (VRTX3-T-0143), before any TASK exists. Per the team contract these are the planner's exclusive responsibility and appear in **no** TASK's scope — including the count bump, which the idea's success metrics assign to implementation but which the contract reserves to the planner. Done here instead. (The ticket API enforces this independently: it stripped a paragraph from each TASK description on creation for merely naming a root doc, and the paragraph was re-added without the reference.)

- **AGENT.md** — probe-family count 74 → 77 under `## Conventions → Health Probe Routes`; the legacy-timing-test figure re-derived (47 of 77) and the copy-source note extended to VRTX3-I-0030. `## Gotchas` records the thirteenth SPA-fallback confirmation, adds the point that a correct canvas is not a substitute for a measurement, and appends `:5001` to the port-drift list. Changelog entry.
- **PRODUCT.md** — `## Features → Health probe endpoints`: count 74 → 77 and the "most recent set" pointer moved to the `568557289` family. Scope, user stories and per-probe acceptance criteria unchanged — this sprint adds instances of an existing feature, not a new feature. Changelog entry.
- **ARCHITECTURE.md** — `## Routing → Health probe route contract`: count 74 → 77; build-output naming example refreshed to a route from this sprint. `## Key Decisions` unchanged (the no-shared-helper decision already governs). Changelog entry.
- **DESIGN.md** — Changelog entry only; the sprint is backend-only and touches nothing in `src/`.

All three counts were re-derived from the filesystem (74 measured + 3 added = 77), not incremented blind.
...
### Phase 1 — `GET /api/healthz-smoke-568557289-a`

Create `routes/api/healthz-smoke-568557289-a.ts` (copy `healthz-smoke-528856326-a.ts`, change the variant string) and `routes/api/healthz-smoke-568557289-a.test.ts` (copy `healthz-smoke-528856326-a.test.ts`, change the import path, imported binding, `describe` title, request URL and expected variant). Two new files, nothing modified.

### Phase 2 — `GET /api/healthz-smoke-568557289-b`

Identical, for `-b`. Two new files, nothing modified.

### Phase 3 — `GET /api/healthz-smoke-568557289-c`
...
| ----- | ------------ | --------------------- | ------------------------------------------------------ |
| —     | VRTX3-T-0144 | EPIC                  | container — closes by rollup                           |
| —     | VRTX3-T-0145 | STORY                 | container — closes by rollup                           |
| 1     | VRTX3-T-0146 | TASK (implementation) | `routes/api/healthz-smoke-568557289-a.ts` + `.test.ts` |
| 2     | VRTX3-T-0147 | TASK (implementation) | `routes/api/healthz-smoke-568557289-b.ts` + `.test.ts` |
| 3     | VRTX3-T-0148 | TASK (implementation) | `routes/api/healthz-smoke-568557289-c.ts` + `.test.ts` |
| 4     | —            | —                     | folded into 1–3                                        |
...
A single agent could plainly write six files in one session, and the minimum-viable-backlog instinct is to merge. Not here. The idea's second user story ("As an implementing agent, I want each endpoint to be one new file plus one new test and zero edits to existing files, so that my branch never conflicts with the other two") and its first success metric ("the three units are implemented on separate branches and merge with **zero conflicts** and zero touched-file overlap") make the three-way independence _the deliverable_. Merging into one ticket would ship the endpoints and drop the thing being tested. Every prior sprint of this shape (SPRINT-0019, VRTX3-S-0004, VRTX3-S-0006, VRTX3-S-0011, VRTX3-S-0013, VRTX3-S-0016, VRTX3-S-0017, VRTX3-S-0019) decomposed the same way.
...
- **A doc-count ticket** — the 74 → 77 bump lives in AGENT.md / ARCHITECTURE.md / PRODUCT.md, which are the planner's exclusive responsibility and were brought to target state on this planning ticket before any TASK existed.
...
- **Dev-server port drift is real and has cost prior sprints time.** Vite bound `:5005`, `:5006`, `:5007`, `:5000` and now `:5001` across the last five sprints of this shape. There is no sequence to extrapolate — read the banner.
...
- **Copying the wrong _test_ source is the third**, and remains one paste away: 47 of the 74 probe tests carry the flaky timing case, so it is still the majority shape in the directory. The idea names the correct pair this time, so the risk is purely a sampling accident; each PLAN.md names `healthz-smoke-528856326-a.test.ts` and states the omission explicitly.
- **The idea's method-agnostic line is correct and is planned to.** These handlers declare no method guard, so every verb returns the same 200 JSON body — measured in VRTX3-S-0008 against a control route. The idea records this under Out of Scope rather than misstating it, so the TASKs simply require that no method guard be added. Adding a 405 to three routes would leave them inconsistent with the other 74.
- **Assumption:** `variant` is the string `"568557289"` for all three, matching every sibling triple (`528856326`, `841017405`, `238855431`, `472035881`) — not a number, and not suffixed per letter. The idea confirms this explicitly in its Open Questions and Assumptions.
- **Assumption:** `middleware/auth.ts` stays a permissive stub. Real rejecting auth would break all 77 probes without any probe file changing.
- **Standing risk:** with 77 near-identical probes committed, an engineer will eventually be tempted to factor out a shared handler. That would convert every future probe from a zero-overlap ticket into a shared-file edit — the exact thing these probes exist to disprove. Recorded in ARCHITECTURE.md § Key Decisions so it stops being re-litigated.
- **Retention is still unanswered, and re-raising it has stopped being useful.** No probe has ever been retired; the family grows ~3 per sprint (74 → 77 this sprint), and `routes/api/` now holds 150 files, heading for 156. A retention-policy backlog TASK has been raised and **cancelled unworked three times** — VRTX3-T-0074 (VRTX3-S-0011), VRTX3-T-0089 (VRTX3-S-0013), VRTX3-T-0111 (VRTX3-S-0016) — alongside VRTX3-T-0104 (remove the flaky timing assertion from the 47 legacy tests) and VRTX3-T-0114, both likewise cancelled. Five cancelled placeholders is enough evidence: raising a sixth would add a paid dispatch and change nothing. VRTX3-S-0017 through -0020 all declined to, and this sprint does the same, recording the question here instead. It needs a human decision, not another ticket. The second-order cost stays visible in the findings above: because those 47 legacy tests were never cleaned up, they remain the majority shape in the directory, which is why an idea canvas sampled one as its template in VRTX3-S-0017.
