# Plan — VRTX-0008: Implement health check endpoint variant A

## Objective

Implement the first independent health check endpoint at `GET /api/healthz-smoke-126862920-a` as a standalone Nitro route with no shared code, no database access, no authentication, and no dependencies on other endpoints.
...
**File**: `routes/api/healthz-smoke-126862920-a.ts`
...
**File**: `routes/api/healthz-smoke-126862920-a.test.ts`

```typescript
import { H3Event } from "nitro/h3";
import { describe, expect, it } from "vitest";

import healthzA from "./healthz-smoke-126862920-a";

describe("GET /api/healthz-smoke-126862920-a", () => {
  it("returns HTTP 200 with correct response body", async () => {
    const event = new H3Event(new Request("http://localhost/api/healthz-smoke-126862920-a"));
...
- New test in `routes/api/healthz-smoke-126862920-a.test.ts` must pass
...
| ---------------------------------------------- | --------- | ------------------------------- |
| `routes/api/healthz-smoke-126862920-a.ts`      | Create    | Route handler for endpoint A    |
| `routes/api/healthz-smoke-126862920-a.test.ts` | Create    | Integration test for endpoint A |

**No other TASKs should modify these files** during this sprint. These files are exclusive to VRTX-0008.

**No file overlap with other TASKs**: VRTX-0009 creates `healthz-smoke-126862920-b.*`, VRTX-0010 creates `healthz-smoke-126862920-c.*`, VRTX-0011 modifies docs only.
...
**Endpoint**: `GET /api/healthz-smoke-126862920-a`
...
- Test file MUST be located at `routes/api/healthz-smoke-126862920-a.test.ts`
...
✅ `routes/api/healthz-smoke-126862920-a.ts` created with correct handler  
✅ `routes/api/healthz-smoke-126862920-a.test.ts` created with passing tests  
✅ `bun run typecheck` passes (no errors)  
✅ `bun run lint` passes (zero warnings)  
✅ `bun run test` passes (new test + all existing tests)  
...
✅ `artifacts/SPRINT-0002/VRTX-0008/PLAN.md` committed
