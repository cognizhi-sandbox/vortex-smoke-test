# Plan — VRTX3-T-0086: Add health probe endpoint `/api/healthz-smoke-841017405-a`
...
`GET /api/healthz-smoke-841017405-a` answers on the running server with `Content-Type: application/json` and a body deep-equal to `{ "ok": true, "variant": "841017405" }`, backed by one self-contained handler file and one colocated Vitest test. The endpoint shares no code with its two siblings (VRTX3-T-0087 / VRTX3-T-0088) and imports nothing beyond `nitro/h3` — no auth, no database, no helper. Done means two new files exist, nothing else in the repository changed, and the route is proven wired by a live request rather than by a unit test alone.

## Steps

1. Copy `routes/api/healthz-smoke-528856326-a.ts` to `routes/api/healthz-smoke-841017405-a.ts`. Change the `variant` string to `"841017405"`. Change nothing else — no `event` parameter, no method guard, no added import.
2. Copy `routes/api/healthz-smoke-528856326-a.test.ts` to `routes/api/healthz-smoke-841017405-a.test.ts`. Update the import path, the imported binding name, the `describe` title, the request URL and the expected variant.
...
5. Start the dev server and issue a real `GET /api/healthz-smoke-841017405-a`. Confirm the response is `application/json` with the exact body. **A `text/html` response means the route did not register** — see Gotchas below.
6. Build, and confirm `.output/server/_routes/api/healthz_smoke_841017405_a.mjs` exists and that no module was built from the `.test.ts` file.
...
| File                                           | Role                                             |
| ---------------------------------------------- | ------------------------------------------------ |
| `routes/api/healthz-smoke-841017405-a.ts`      | the handler — the filename _is_ the URL contract |
| `routes/api/healthz-smoke-841017405-a.test.ts` | colocated `H3Event` integration test             |

**Must not touch:** `routes/api/healthz-smoke-841017405-b.*` and `-c.*` (owned by VRTX3-T-0087 and VRTX3-T-0088), any of the 50 pre-existing probes, `vite.config.ts`, `vitest.config.ts`, `middleware/auth.ts`, `server.ts`, `db/`, `drizzle/`, `src/`, `e2e/`, `package.json`, or `.github/workflows/`.
...
```ts
// routes/api/healthz-smoke-841017405-a.ts
...
- **Route path:** `/api/healthz-smoke-841017405-a`, derived from the filename. No registration step exists; a filename typo produces a wrong URL with no other symptom.
