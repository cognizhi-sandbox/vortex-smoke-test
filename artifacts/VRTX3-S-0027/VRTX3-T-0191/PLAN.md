...
# Plan — VRTX3-T-0191: GET /api/healthz-smoke-868033827-c

## Objective

`GET /api/healthz-smoke-868033827-c` answers with `Content-Type: application/json` and a body deep-equal to `{ "ok": true, "variant": "868033827" }`, served by a new self-contained Nitro handler at `routes/api/healthz-smoke-868033827-c.ts` with a colocated integration test beside it. Two new files, zero modified files. This ticket shares no file with VRTX3-T-0189 or VRTX3-T-0190 and has no ordering relationship to either — build and merge it whenever you pick it up.
...
1. Copy `routes/api/healthz-smoke-528856326-a.ts` → `routes/api/healthz-smoke-868033827-c.ts` and change the `variant` string to `"868033827"`. Change nothing else — no `event` parameter, no method guard, no extra import.
2. Copy `routes/api/healthz-smoke-528856326-a.test.ts` → `routes/api/healthz-smoke-868033827-c.test.ts` and update five things: the import path, the imported binding name, the `describe` title, the request URL, and the expected `variant`.
...
| File                                           | Action | Notes                                        |
| ---------------------------------------------- | ------ | -------------------------------------------- |
| `routes/api/healthz-smoke-868033827-c.ts`      | CREATE | The handler. Owned solely by this ticket.    |
| `routes/api/healthz-smoke-868033827-c.test.ts` | CREATE | Colocated test. Owned solely by this ticket. |
...
- **URL:** `/api/healthz-smoke-868033827-c`, character for character. The filename **is** the URL registration; there is no route table, so a typo is a wrong URL with no other symptom.
- **Response body:** exactly `{ ok: true, variant: "868033827" }` — `ok` is the boolean `true`, `variant` is the **string** `"868033827"` (not a number, and not suffixed with `-c`).
...
- `routes/api/healthz-smoke-868033827-c.ts` exists, default-exports a `defineHandler` from `nitro/h3` taking no parameters, and returns the literal `{ ok: true, variant: "868033827" }`.
- A live request to `/api/healthz-smoke-868033827-c` returns `Content-Type: application/json` with a body deep-equal to `{"ok":true,"variant":"868033827"}` — not the 949-byte `text/html` SPA shell.
- `routes/api/healthz-smoke-868033827-c.test.ts` exists, imports the handler directly, constructs an `H3Event` for `http://localhost/api/healthz-smoke-868033827-c`, and asserts the returned object deep-equals `{ ok: true, variant: "868033827" }`; it contains exactly one `it()` case and no elapsed-time or "responds in under Nms" assertion.
...
- The production build emits `.output/server/_routes/api/healthz_smoke_868033827_c.mjs` (dashes → underscores), and no `*.test.ts` file appears in the bundle.
...
| ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------ |
| New colocated `routes/api/healthz-smoke-868033827-c.test.ts`       | Handler returns `{ ok: true, variant: "868033827" }` — passes                                                |
| The 89 pre-existing probe tests + the rest of the unit suite       | Unchanged, still green (this ticket touches none of their files)                                             |
| Live request to `/api/healthz-smoke-868033827-c` on the dev server | `application/json` + the exact JSON body (proves Nitro registered it)                                        |
| Production build                                                   | Succeeds; emits `.output/server/_routes/api/healthz_smoke_868033827_c.mjs`, and no `*.test.ts` in the bundle |
