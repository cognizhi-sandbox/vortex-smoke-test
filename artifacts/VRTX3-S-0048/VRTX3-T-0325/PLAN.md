# PLAN — VRTX3-T-0325: /api/healthz-smoke-956166896-b
...
Serve `GET /api/healthz-smoke-956166896-b` with `200 application/json` and the literal body
...
1. Copy `routes/api/healthz-smoke-528856326-a.ts` to `routes/api/healthz-smoke-956166896-b.ts` and
...
2. Copy `routes/api/healthz-smoke-528856326-a.test.ts` to
   `routes/api/healthz-smoke-956166896-b.test.ts`; change the import path, the identifier, the
...
4. Confirm the route module appears in the production build output at
   `.output/server/_routes/api/healthz_smoke_956166896_b.mjs` (`design.md` § D5).
...
- `routes/api/healthz-smoke-956166896-b.ts`
- `routes/api/healthz-smoke-956166896-b.test.ts`

Read-only: `routes/api/healthz-smoke-528856326-a.ts`, `routes/api/healthz-smoke-528856326-a.test.ts`,
`vite.config.ts`, `vitest.config.ts`.

No overlap with VRTX3-T-0324 or VRTX3-T-0326, so there is no dependency edge between the three.
