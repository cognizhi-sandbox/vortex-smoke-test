# VRTX3-T-0092 — Fix plan

**Sprint:** VRTX3-S-0014 · **Type:** DEFECT · **Route:** `/api/healthz-smoke-bugfix-174694844` · **Variant:** `"174694844"`
...
```
GET /api/healthz-smoke-bugfix-174694844
...
```bash
$ grep -rn "174694844" . --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=artifacts
...
| Filename typo under a near-miss name       | Repo-wide grep for `174694844` returns zero matches                                                                 |
...
1. `routes/api/healthz-smoke-bugfix-174694844.ts` — default-export a single `defineHandler` imported from `nitro/h3`, taking
   no parameters, returning the literal `{ ok: true, variant: "174694844" }`.
2. `routes/api/healthz-smoke-bugfix-174694844.test.ts` — mirror the sibling spec: construct an `H3Event` from a `Request` for
   the route's URL, call the handler, and assert the resolved value equals
   `{ ok: true, variant: "174694844" }`. **One assertion only.**

**Fixed interface contract — do not deviate:**

```ts
// routes/api/healthz-smoke-bugfix-174694844.ts
...
- `routes/api/healthz-smoke-bugfix-174694844.ts` exists and default-exports a single `defineHandler` from `nitro/h3` taking
  no parameters, matching the contract above exactly.
- `GET /api/healthz-smoke-bugfix-174694844` on a freshly started server responds with `Content-Type: application/json` and
  the body exactly `{"ok":true,"variant":"174694844"}` — no extra keys, `variant` a string not a number.
  The route table is built at scan time, so an already-running server must be restarted first.
- `routes/api/healthz-smoke-bugfix-174694844.test.ts` exists, constructs an `H3Event` for the route's URL, and its single
  assertion — the handler resolves to `{ ok: true, variant: "174694844" }` — passes. It carries no
  elapsed-time assertion.
- The production build emits `.output/server/_routes/api/healthz_smoke_bugfix_174694844.mjs`, and no `*.test.ts`-derived
...
| File                                                | Action |
| --------------------------------------------------- | ------ |
| `routes/api/healthz-smoke-bugfix-174694844.ts`      | create |
| `routes/api/healthz-smoke-bugfix-174694844.test.ts` | create |
...
for `174694844` returns zero matches, so the new filename cannot shadow or be shadowed by an existing
