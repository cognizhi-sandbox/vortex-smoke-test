# VRTX3-T-0094 — Fix plan

**Sprint:** VRTX3-S-0014 · **Type:** DEFECT · **Route:** `/api/healthz-smoke-bugfix3-404580234` · **Variant:** `"404580234"`
...
Measured on this branch against a live `bun run dev` on 2026-08-10:

```
GET /api/healthz-smoke-bugfix3-404580234
  → 200  text/html; charset=utf-8        body: <!doctype html><html lang="en">…   (the SPA shell)

GET /api/healthz-smoke-841017405-a               (control — this route exists)
  → 200  application/json;charset=UTF-8   body: {"ok":true,"variant":"841017405"}
```

**The ticket's reported symptom "404" is incorrect.** An unmatched `/api/*` path is answered by the
SPA `index.html` fallback with **HTTP 200**, in dev and in the production build alike. The defect is
real; only its stated status code is wrong. Consequence for this ticket: a `404 → 200` assertion
proves nothing, because the path already returns 200. **Verify on the response body and
`Content-Type`.**
...
```bash
$ grep -rn "404580234" . --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=artifacts
...
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------- |
| Nitro directory scanning disabled          | `nitro({ serverDir: "./", ignore: ["**/*.test.ts"] })` is set at `vite.config.ts:29`, and 54 sibling probes resolve |
...
| Filename typo under a near-miss name       | Repo-wide grep for `404580234` returns zero matches                                                                 |
| Middleware short-circuiting the request    | `middleware/auth.ts` only sets `event.context.user`; it never returns a response                                    |
| nginx dropping the path                    | `nginx.conf` proxies all of `/api/` through with no per-route allowlist                                             |
...
1. `routes/api/healthz-smoke-bugfix3-404580234.ts` — default-export a single `defineHandler` imported from `nitro/h3`, taking
   no parameters, returning the literal `{ ok: true, variant: "404580234" }`.
2. `routes/api/healthz-smoke-bugfix3-404580234.test.ts` — mirror the sibling spec: construct an `H3Event` from a `Request` for
   the route's URL, call the handler, and assert the resolved value equals
   `{ ok: true, variant: "404580234" }`. **One assertion only.**

**Fixed interface contract — do not deviate:**

```ts
// routes/api/healthz-smoke-bugfix3-404580234.ts
...
- `routes/api/healthz-smoke-bugfix3-404580234.ts` exists and default-exports a single `defineHandler` from `nitro/h3` taking
  no parameters, matching the contract above exactly.
- `GET /api/healthz-smoke-bugfix3-404580234` on a freshly started server responds with `Content-Type: application/json` and
  the body exactly `{"ok":true,"variant":"404580234"}` — no extra keys, `variant` a string not a number.
  The route table is built at scan time, so an already-running server must be restarted first.
- `routes/api/healthz-smoke-bugfix3-404580234.test.ts` exists, constructs an `H3Event` for the route's URL, and its single
  assertion — the handler resolves to `{ ok: true, variant: "404580234" }` — passes. It carries no
  elapsed-time assertion.
- The production build emits `.output/server/_routes/api/healthz_smoke_bugfix3_404580234.mjs`, and no `*.test.ts`-derived
...
| File                                                 | Action |
| ---------------------------------------------------- | ------ |
| `routes/api/healthz-smoke-bugfix3-404580234.ts`      | create |
| `routes/api/healthz-smoke-bugfix3-404580234.test.ts` | create |

No other file is in scope. This map is disjoint from the other two defects in VRTX3-S-0014, so this
...
for `404580234` returns zero matches, so the new filename cannot shadow or be shadowed by an existing
route. No shared module, middleware interaction, schema change or migration is involved.
`e2e/smoke.spec.ts` probes `/api/hello`, not this family, so E2E impact is nil.
