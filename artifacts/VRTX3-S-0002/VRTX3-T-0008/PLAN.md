...
# Plan — VRTX3-T-0008: `/api/healthz-smoke-bugfix2-142310404` returns the SPA shell, not its probe body

> **This directory also contains `fix-note.md` and `tdd-test-result.md` from a DIFFERENT sprint.**
> The `VRTX3-S-0002` sprint key and the `VRTX3-T-0008` ticket key were both recycled; the files
> alongside this plan were committed in `e167bb8` and describe variant `524723214`, which is not
> yours. Ignore them — this `PLAN.md` is the only current file here. See `SPRINT-PLAN.md` note 1.

## Objective

`GET /api/healthz-smoke-bugfix2-142310404` answers HTTP 200 with `Content-Type: application/json` and
a body deep-equal to `{ "ok": true, "variant": "142310404" }`, served by a new self-contained Nitro
route with a colocated test. Two new files; no existing source file changes.

## Root cause

**The handler was never written.** `routes/api/healthz-smoke-bugfix2-142310404.ts` does not exist,
...
A repo-wide grep for `142310404` (excluding `node_modules`/`.git`) returned zero matches, confirming
...
```
/api/healthz-smoke-bugfix2-142310404   200 text/html; charset=utf-8       949b  (SPA index.html shell)
...
1. Copy `routes/api/healthz-smoke-528856326-a.ts` to
   `routes/api/healthz-smoke-bugfix2-142310404.ts`, changing only the variant string to
   `"142310404"`. Change nothing else — no `event` parameter, no method guard, no import beyond
   `nitro/h3`.
2. Copy `routes/api/healthz-smoke-528856326-a.test.ts` to
   `routes/api/healthz-smoke-bugfix2-142310404.test.ts`, updating the import path, the `describe`
...
- `routes/api/healthz-smoke-bugfix2-142310404.ts`
- `routes/api/healthz-smoke-bugfix2-142310404.test.ts`

Modifies nothing. **Does not intersect VRTX3-T-0007 or VRTX3-T-0009** — no `depends_on` in any
...
- Route path: `/api/healthz-smoke-bugfix2-142310404` — the filename is the URL; a typo is a dead
  route with no other symptom. Note the **`-bugfix2-`** infix: the variant id alone does not
  distinguish this route from its two siblings this sprint, which use `-bugfix-` and `-bugfix3-`.
- `variant` is the **string** `"142310404"`, not a number, and carries **no** `bugfix2` prefix — the
...
- DoD-1: `routes/api/healthz-smoke-bugfix2-142310404.ts` exists and default-exports a single
  `defineHandler` from `nitro/h3` returning the literal `{ ok: true, variant: "142310404" }`.
- DoD-2: A live `GET /api/healthz-smoke-bugfix2-142310404` against a running dev server returns HTTP
  200 with `Content-Type: application/json` and body exactly `{"ok":true,"variant":"142310404"}` —
...
- DoD-3: In the same session and on the same port, control route `/api/healthz-smoke-528856326-a`
  ...
  server as a pass.
- DoD-4: `routes/api/healthz-smoke-bugfix2-142310404.test.ts` constructs an `H3Event` and asserts
  `toEqual({ ok: true, variant: "142310404" })` as its single case, with no timing assertion.
- DoD-5: No file outside the two new ones is added, modified or deleted, and no dependency is added.

## Test plan

- **API integration**, `routes/api/healthz-smoke-bugfix2-142310404.test.ts` — one case, collected by
  the Vitest `server` project (`environment: "node"`) with no configuration change. Expected: the
  handler's returned object deep-equals `{ ok: true, variant: "142310404" }`. Covers DoD-1, DoD-4.
