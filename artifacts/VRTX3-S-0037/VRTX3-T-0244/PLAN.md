...
downstream: [artifacts/VRTX3-S-0037/VRTX3-T-0244/tdd-test-result.md]
---

# Plan — VRTX3-T-0244: Add the missing `/api/healthz-smoke-bugfix2-386341015` probe

## Objective

`GET /api/healthz-smoke-bugfix2-386341015` answers with `Content-Type: application/json` and the body
`{"ok":true,"variant":"386341015"}`. Today that path is unrouted and is answered by the SPA
...
```
/api/healthz-smoke-bugfix2-386341015  →  200 text/html; charset=utf-8   949 B  (SPA shell)
/api/healthz-smoke-528856326-a        →  200 application/json;charset=UTF-8  33 B  {"ok":true,"variant":"528856326"}
```

A repo-wide grep for `386341015` (`*.ts`, `*.tsx`, `*.md`, `*.json`, excluding `node_modules`)
...
1. Create `routes/api/healthz-smoke-bugfix2-386341015.ts`: default-export a `defineHandler` imported
   from `nitro/h3`, taking no parameters, returning the literal `{ ok: true, variant: "386341015" }`.
   Copy `routes/api/healthz-smoke-528856326-a.ts` and change only the variant string.
2. Create `routes/api/healthz-smoke-bugfix2-386341015.test.ts` by copying
   `routes/api/healthz-smoke-528856326-a.test.ts` — one `it()` case that builds
   `new H3Event(new Request("http://localhost/api/healthz-smoke-bugfix2-386341015"))`, awaits the
   imported handler, and asserts `toEqual({ ok: true, variant: "386341015" })`.
3. Check the literal `386341015` agrees in three places: the handler filename, the test filename,
   and the `variant` field. Note the filename says `bugfix2` while the variant is bare `386341015` —
   the `bugfix2` prefix is part of the path, never part of the `variant` string. The filename _is_
   the URL contract; a typo ships a route that is unreachable while every test still passes.

## File/module ownership

- `routes/api/healthz-smoke-bugfix2-386341015.ts` (new)
- `routes/api/healthz-smoke-bugfix2-386341015.test.ts` (new)

No other file. No overlap with VRTX3-T-0243 or VRTX3-T-0245, so no `depends_on` edge. The root docs
...
```ts
// routes/api/healthz-smoke-bugfix2-386341015.ts
...
- Route path: `/api/healthz-smoke-bugfix2-386341015` (derived from the filename; the `/api/` prefix
  comes from the `routes/api/` directory and is not optional).
- Response body: exactly `{ ok: true, variant: "386341015" }` — `variant` is the **string**
  `"386341015"` with no `bugfix2` prefix, no extra keys.
...
- DoD-1: `routes/api/healthz-smoke-bugfix2-386341015.ts` exists and default-exports a `defineHandler`
  from `nitro/h3` returning `{ ok: true, variant: "386341015" }`.
- DoD-2: `routes/api/healthz-smoke-bugfix2-386341015.test.ts` exists, builds a real `H3Event`, calls
  the handler directly, and asserts the exact body. It has exactly one `it()` case.
- DoD-3: A live request to `/api/healthz-smoke-bugfix2-386341015` on a running dev server returns
  `Content-Type: application/json` with body `{"ok":true,"variant":"386341015"}` — asserted on the
...
- DoD-4: The route compiles into the production server — a module for this path appears under
  ...
  `healthz_smoke_bugfix2_386341015.mjs`).
...
- API-route test, `routes/api/healthz-smoke-bugfix2-386341015.test.ts` — covers DoD-1, DoD-2.
