...
# Plan — VRTX3-T-0232: GET /api/healthz-smoke-180848429-c

## Objective

`GET /api/healthz-smoke-180848429-c` answers with `Content-Type: application/json` and a body deep-equal to `{ "ok": true, "variant": "180848429" }`, served by a new self-contained Nitro handler at `routes/api/healthz-smoke-180848429-c.ts` with a colocated integration test beside it. Two new files, zero modified files.

This ticket shares no file with VRTX3-T-0230 or VRTX3-T-0231 and has no ordering relationship to either — build and merge it whenever you pick it up.
...
1. Copy `routes/api/healthz-smoke-528856326-a.ts` → `routes/api/healthz-smoke-180848429-c.ts` and change the `variant` string to `"180848429"`. Change nothing else — no `event` parameter, no method guard, no extra import.

2. Copy `routes/api/healthz-smoke-528856326-a.test.ts` → `routes/api/healthz-smoke-180848429-c.test.ts` and update five things: the import path, the imported binding name, the `describe` title, the request URL, and the expected `variant`.
...
| File                                           | Action | Notes                                        |
| ---------------------------------------------- | ------ | -------------------------------------------- |
| `routes/api/healthz-smoke-180848429-c.ts`      | CREATE | The handler. Owned solely by this ticket.    |
| `routes/api/healthz-smoke-180848429-c.test.ts` | CREATE | Colocated test. Owned solely by this ticket. |
...
- **URL:** `/api/healthz-smoke-180848429-c`, character for character. The filename **is** the URL registration; there is no route table, so a typo is a wrong URL with no other symptom.
- **Response body:** exactly `{ ok: true, variant: "180848429" }` — `ok` is the boolean `true`, `variant` is the **string** `"180848429"` (not a number, and not suffixed with `-c`). All three probes in this sprint carry the identical variant string; the `a`/`b`/`c` distinction lives in the URL only.
...
| Live request to `/api/healthz-smoke-180848429-c` on the dev server | `application/json` + the exact JSON body (proves Nitro registered it)                                        |
| Production build                                                   | Succeeds; emits `.output/server/_routes/api/healthz_smoke_180848429_c.mjs`, and no `*.test.ts` in the bundle |
