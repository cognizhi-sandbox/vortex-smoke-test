# Plan — VRTX3-T-0163: GET /api/healthz-smoke-1065915107-b

## Objective

`GET /api/healthz-smoke-1065915107-b` answers with `Content-Type: application/json` and a body deep-equal to `{ "ok": true, "variant": "1065915107" }`, served by a new self-contained Nitro handler at `routes/api/healthz-smoke-1065915107-b.ts` with a colocated integration test beside it. Two new files, zero modified files. This ticket shares no file with VRTX3-T-0162 or VRTX3-T-0164 and has no ordering relationship to either — build and merge it whenever you pick it up.
...
1. Copy `routes/api/healthz-smoke-528856326-a.ts` → `routes/api/healthz-smoke-1065915107-b.ts` and change the `variant` string to `"1065915107"`. Change nothing else — no `event` parameter, no method guard, no extra import.
2. Copy `routes/api/healthz-smoke-528856326-a.test.ts` → `routes/api/healthz-smoke-1065915107-b.test.ts` and update five things: the import path, the imported binding name, the `describe` title, the request URL, and the expected `variant`.
...
| File                                            | Action | Notes                                        |
| ----------------------------------------------- | ------ | -------------------------------------------- |
| `routes/api/healthz-smoke-1065915107-b.ts`      | CREATE | The handler. Owned solely by this ticket.    |
| `routes/api/healthz-smoke-1065915107-b.test.ts` | CREATE | Colocated test. Owned solely by this ticket. |
...
- **URL:** `/api/healthz-smoke-1065915107-b`, character for character. The filename **is** the URL registration; there is no route table, so a typo is a wrong URL with no other symptom.
- **Response body:** exactly `{ ok: true, variant: "1065915107" }` — `ok` is the boolean `true`, `variant` is the **string** `"1065915107"` (not a number, and not suffixed with `-b`).
...
- `routes/api/healthz-smoke-1065915107-b.ts` exists, default-exports a `defineHandler` from `nitro/h3` taking no parameters, and returns the literal `{ ok: true, variant: "1065915107" }`.
- A live request to `/api/healthz-smoke-1065915107-b` returns `Content-Type: application/json` with a body deep-equal to `{"ok":true,"variant":"1065915107"}` — not the 949-byte `text/html` SPA shell.
- `routes/api/healthz-smoke-1065915107-b.test.ts` exists, imports the handler directly, constructs an `H3Event` for `http://localhost/api/healthz-smoke-1065915107-b`, and asserts the returned object deep-equals `{ ok: true, variant: "1065915107" }`; it contains exactly one `it()` case and no elapsed-time or "responds in under Nms" assertion.
...
- The production build emits `.output/server/_routes/api/healthz_smoke_1065915107_b.mjs` (dashes → underscores), and no `*.test.ts` file appears in the bundle.
...
| ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| New colocated `routes/api/healthz-smoke-1065915107-b.test.ts`       | Handler returns `{ ok: true, variant: "1065915107" }` — passes                                                |
| The 80 pre-existing probe tests + the rest of the unit suite        | Unchanged, still green (this ticket touches none of their files)                                              |
| Live request to `/api/healthz-smoke-1065915107-b` on the dev server | `application/json` + the exact JSON body (proves Nitro registered it)                                         |
| Production build                                                    | Succeeds; emits `.output/server/_routes/api/healthz_smoke_1065915107_b.mjs`, and no `*.test.ts` in the bundle |
