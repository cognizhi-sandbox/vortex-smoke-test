...
upstream: [artifacts/VRTX3-S-0040/qa-test-report.md]
---

# Release notes — VRTX3-S-0040

## Added

- `GET /api/healthz-smoke-503463873-a` answers `200` with `{"ok":true,"variant":"503463873"}` as
  `application/json`. Usable as a liveness check — it needs no authentication and does not touch the
  database, so it still answers when either is unavailable. (VRTX3-T-0268)
- `GET /api/healthz-smoke-503463873-b` — same response and same guarantees. (VRTX3-T-0269)
- `GET /api/healthz-smoke-503463873-c` — same response and same guarantees. (VRTX3-T-0270)
...
default changed, and no migration to run.
...
Everything VRTX3-I-0049 placed out of scope is genuinely absent, by design rather than by omission:
...
Verified at integration QA — see `artifacts/VRTX3-S-0040/qa-test-report.md` (PASS: 15/15 spec
...
| ------------------------------ | -------------------------------------------- | ---------------------------------------------------------- | --------- | --------- |
| Release contents recorded      | this file                                    | `artifacts/VRTX3-S-0040/release-notes.md`                  | Satisfied | —         |
| Release verified before land   | QA PASS verdict                              | `artifacts/VRTX3-S-0040/qa-test-report.md`                 | Satisfied | —         |
| Known limitations communicated | scope exclusions + probe-check caveat, above | this file § Not included, § Upgrade notes                  | Satisfied | —         |
| Open defects at release        | 0 found, 0 open                              | `artifacts/VRTX3-S-0040/integration-defects-resolution.md` | Satisfied | —         |
