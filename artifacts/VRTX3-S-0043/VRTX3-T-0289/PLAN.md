...
# Plan — VRTX3-T-0289: Add the missing `/api/healthz-smoke-bugfix-507266122` probe

## Objective

`GET /api/healthz-smoke-bugfix-507266122` answers with `Content-Type: application/json` and the body
`{"ok":true,"variant":"507266122"}`. Today that path is unrouted and is answered by the SPA
...
```
/api/healthz-smoke-bugfix-507266122  →  200 text/html; charset=utf-8            949 B  (SPA shell)
/api/healthz-smoke-528856326-a       →  200 application/json;charset=UTF-8       33 B  {"ok":true,"variant":"528856326"}
```

`ls routes/api/ | grep 507266122` returns nothing and `git log --all -S'507266122'` returns zero
...
to dispatch to. Sibling probes resolve correctly under the same config, so nothing is misconfigured.
...
1. Create `routes/api/healthz-smoke-bugfix-507266122.ts` by copying
...
2. Create `routes/api/healthz-smoke-bugfix-507266122.test.ts` by copying
   `routes/api/healthz-smoke-528856326-a.test.ts` — one `it()` case that builds
   `new H3Event(new Request("http://localhost/api/healthz-smoke-bugfix-507266122"))`, awaits the
   imported handler, and asserts `toEqual({ ok: true, variant: "507266122" })`. Add the regression
   header comment fixed below. **Do not copy `healthz-smoke-bugfix-1054626998.test.ts` or any other
   `bugfix*` test** — 33 of the 62 tests in that subfamily carry a wall-clock timing case.
3. Check the literal `507266122` agrees in three places: the handler filename, the test filename,
...
- `routes/api/healthz-smoke-bugfix-507266122.ts` (new)
- `routes/api/healthz-smoke-bugfix-507266122.test.ts` (new)

No other file. No overlap with VRTX3-T-0290 or VRTX3-T-0291, so no `depends_on` edge. The root docs
...
```ts
// routes/api/healthz-smoke-bugfix-507266122.ts
...
```ts
// routes/api/healthz-smoke-bugfix-507266122.test.ts — header comment, fixed text
/**
...
- Route path: `/api/healthz-smoke-bugfix-507266122` (derived from the filename; the `/api/` prefix
  comes from the `routes/api/` directory and is not optional). No `-a`/`-b`/`-c` suffix — no file in
  the `bugfix*` subfamily uses one.
- Response body: exactly `{ ok: true, variant: "507266122" }` — `variant` is the **string**
  `"507266122"` (the numeric part only, without the `bugfix` prefix), no extra keys.
...
- DoD-1: `routes/api/healthz-smoke-bugfix-507266122.ts` exists and default-exports a `defineHandler`
  from `nitro/h3` returning `{ ok: true, variant: "507266122" }`.
- DoD-2: `routes/api/healthz-smoke-bugfix-507266122.test.ts` exists, carries the regression header
  comment above, builds a real `H3Event`, calls the handler directly, and asserts the exact body. It
  has exactly one `it()` case.
- DoD-3: A live request to `/api/healthz-smoke-bugfix-507266122` on a running dev server returns
  `Content-Type: application/json` with body `{"ok":true,"variant":"507266122"}` — asserted on the
...
- API-route test, `routes/api/healthz-smoke-bugfix-507266122.test.ts` — covers DoD-1, DoD-2.
