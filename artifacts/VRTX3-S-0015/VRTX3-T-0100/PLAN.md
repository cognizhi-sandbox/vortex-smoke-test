# VRTX3-T-0100 — Fix plan

**Sprint:** VRTX3-S-0015 · **Type:** DEFECT · **Route:** `/api/healthz-smoke-bugfix3-418626414` · **Variant:** `"418626414"`
...
Measured on this branch against a live dev server on 2026-08-10:

```
GET /api/healthz-smoke-bugfix3-418626414
  → 200  text/html; charset=utf-8        body: <!doctype html><html lang="en">…   (the SPA shell)

GET /api/healthz-smoke-bugfix3-404580234         (control — this route exists)
  → 200  application/json;charset=UTF-8   body: {"ok":true,"variant":"404580234"}
```

**The ticket's reported symptom "404" is incorrect.** The idea canvas for VRTX3-I-0024 already
flagged this as an assumption and asked for a measurement — this is it, and it confirms the
canvas's correction. An unmatched `/api/*` path is answered by the SPA `index.html` fallback with
**HTTP 200**, in dev and in the production build alike — `nginx.conf` proxies `location /api/`
straight to Nitro and does not set `proxy_intercept_errors`, so it does not change this either. The
defect is real; only its stated status code is wrong. Consequence for this ticket: a `404 → 200`
assertion proves nothing, because the path already returns 200. **Verify on the response body and
`Content-Type`.**
...
```bash
$ grep -rl "418626414" . --exclude-dir=node_modules --exclude-dir=.git
...
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------- |
| Nitro directory scanning disabled          | `nitro({ serverDir: "./", ignore: ["**/*.test.ts"] })` is set at `vite.config.ts:29`, and 56 sibling probes resolve |
...
| Filename typo under a near-miss name       | Repo-wide grep for `418626414` returns zero matches                                                                 |
...
1. `routes/api/healthz-smoke-bugfix3-418626414.ts` — default-export a single `defineHandler`
   imported from `nitro/h3`, taking no parameters, returning the literal
   `{ ok: true, variant: "418626414" }`.
2. `routes/api/healthz-smoke-bugfix3-418626414.test.ts` — mirror the sibling spec: construct an
   `H3Event` from a `Request` for the route's URL, call the handler, and assert the resolved value
   equals `{ ok: true, variant: "418626414" }`. **One assertion only.**

**Fixed interface contract — do not deviate:**

```ts
// routes/api/healthz-smoke-bugfix3-418626414.ts
...
- No method guard. The siblings have none, so every HTTP verb returns the same body; keep it. The
  canvas puts non-`GET` method handling out of scope, and that is the line to plan to.
...
- Root docs (`AGENT.md` / `PRODUCT.md` / `ARCHITECTURE.md` / `DESIGN.md`) are **out of scope**; they
  were brought to target state on the planning ticket. In particular, the canvas's "bump the probe
  count to 57" note is **not** this ticket's work — see the sprint index's Follow-ups section.

## 4. Definition of Done

- `routes/api/healthz-smoke-bugfix3-418626414.ts` exists and default-exports a single
  `defineHandler` from `nitro/h3` taking no parameters, matching the contract above exactly.
- `GET /api/healthz-smoke-bugfix3-418626414` on a freshly started server responds with
  `Content-Type: application/json` and the body exactly `{"ok":true,"variant":"418626414"}` — no
...
- `routes/api/healthz-smoke-bugfix3-418626414.test.ts` exists, constructs an `H3Event` for the
  route's URL, and its single assertion — the handler resolves to
  `{ ok: true, variant: "418626414" }` — passes. It carries no elapsed-time assertion.
- The production build emits `.output/server/_routes/api/healthz_smoke_bugfix3_418626414.mjs`, and
...
| File                                                 | Action |
| ---------------------------------------------------- | ------ |
| `routes/api/healthz-smoke-bugfix3-418626414.ts`      | create |
| `routes/api/healthz-smoke-bugfix3-418626414.test.ts` | create |

No other file is in scope. This map is disjoint from the other two defects in VRTX3-S-0015, so this
...
for `418626414` returns zero matches, so the new filename cannot shadow or be shadowed by an
