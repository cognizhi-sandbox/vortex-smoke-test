# VRTX3-T-0099 — Fix plan

**Sprint:** VRTX3-S-0015 · **Type:** DEFECT · **Route:** `/api/healthz-smoke-bugfix2-487405332` · **Variant:** `"487405332"`
...
```
GET /api/healthz-smoke-bugfix2-487405332
...
```bash
$ grep -rl "487405332" . --exclude-dir=node_modules --exclude-dir=.git
...
| Filename typo under a near-miss name       | Repo-wide grep for `487405332` returns zero matches                                                                 |
...
1. `routes/api/healthz-smoke-bugfix2-487405332.ts` — default-export a single `defineHandler`
   imported from `nitro/h3`, taking no parameters, returning the literal
   `{ ok: true, variant: "487405332" }`.
2. `routes/api/healthz-smoke-bugfix2-487405332.test.ts` — mirror the sibling spec: construct an
   `H3Event` from a `Request` for the route's URL, call the handler, and assert the resolved value
   equals `{ ok: true, variant: "487405332" }`. **One assertion only.**

**Fixed interface contract — do not deviate:**

```ts
// routes/api/healthz-smoke-bugfix2-487405332.ts
...
- `routes/api/healthz-smoke-bugfix2-487405332.ts` exists and default-exports a single
  `defineHandler` from `nitro/h3` taking no parameters, matching the contract above exactly.
- `GET /api/healthz-smoke-bugfix2-487405332` on a freshly started server responds with
  `Content-Type: application/json` and the body exactly `{"ok":true,"variant":"487405332"}` — no
...
- `routes/api/healthz-smoke-bugfix2-487405332.test.ts` exists, constructs an `H3Event` for the
  route's URL, and its single assertion — the handler resolves to
  `{ ok: true, variant: "487405332" }` — passes. It carries no elapsed-time assertion.
- The production build emits `.output/server/_routes/api/healthz_smoke_bugfix2_487405332.mjs`, and
...
| File                                                 | Action |
| ---------------------------------------------------- | ------ |
| `routes/api/healthz-smoke-bugfix2-487405332.ts`      | create |
| `routes/api/healthz-smoke-bugfix2-487405332.test.ts` | create |
...
for `487405332` returns zero matches, so the new filename cannot shadow or be shadowed by an
