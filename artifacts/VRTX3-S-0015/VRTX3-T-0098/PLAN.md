# VRTX3-T-0098 — Fix plan

**Sprint:** VRTX3-S-0015 · **Type:** DEFECT · **Route:** `/api/healthz-smoke-bugfix-406186407` · **Variant:** `"406186407"`
...
```
GET /api/healthz-smoke-bugfix-406186407
...
SPA `index.html` fallback with **HTTP 200**, in dev and in the production build alike — `nginx.conf`
...
```bash
$ grep -rl "406186407" . --exclude-dir=node_modules --exclude-dir=.git
...
| Filename typo under a near-miss name       | Repo-wide grep for `406186407` returns zero matches                                                                 |
...
1. `routes/api/healthz-smoke-bugfix-406186407.ts` — default-export a single `defineHandler` imported
   from `nitro/h3`, taking no parameters, returning the literal `{ ok: true, variant: "406186407" }`.
2. `routes/api/healthz-smoke-bugfix-406186407.test.ts` — mirror the sibling spec: construct an
   `H3Event` from a `Request` for the route's URL, call the handler, and assert the resolved value
   equals `{ ok: true, variant: "406186407" }`. **One assertion only.**

**Fixed interface contract — do not deviate:**

```ts
// routes/api/healthz-smoke-bugfix-406186407.ts
...
- `routes/api/healthz-smoke-bugfix-406186407.ts` exists and default-exports a single `defineHandler`
  from `nitro/h3` taking no parameters, matching the contract above exactly.
- `GET /api/healthz-smoke-bugfix-406186407` on a freshly started server responds with
  `Content-Type: application/json` and the body exactly `{"ok":true,"variant":"406186407"}` — no
...
- `routes/api/healthz-smoke-bugfix-406186407.test.ts` exists, constructs an `H3Event` for the
  route's URL, and its single assertion — the handler resolves to
  `{ ok: true, variant: "406186407" }` — passes. It carries no elapsed-time assertion.
- The production build emits `.output/server/_routes/api/healthz_smoke_bugfix_406186407.mjs`, and no
...
| File                                                | Action |
| --------------------------------------------------- | ------ |
| `routes/api/healthz-smoke-bugfix-406186407.ts`      | create |
| `routes/api/healthz-smoke-bugfix-406186407.test.ts` | create |
...
for `406186407` returns zero matches, so the new filename cannot shadow or be shadowed by an
