# VRTX3-T-0137 — Fix plan

**Sprint:** VRTX3-S-0020 · **Type:** DEFECT · **Route:** `/api/healthz-smoke-bugfix-1060413982` · **Variant:** `"1060413982"`
...
```
GET /api/healthz-smoke-bugfix-1060413982
...
```bash
$ grep -rl "1060413982" . --exclude-dir=node_modules --exclude-dir=.git
...
| Filename typo under a near-miss name       | Repo-wide grep for `1060413982` returns zero matches                                                                |
...
1. `routes/api/healthz-smoke-bugfix-1060413982.ts` — default-export a single `defineHandler`
   imported from `nitro/h3`, taking no parameters, returning the literal
   `{ ok: true, variant: "1060413982" }`.
2. `routes/api/healthz-smoke-bugfix-1060413982.test.ts` — mirror the sibling spec: construct an
   `H3Event` from a `Request` for the route's URL, call the handler, and assert the resolved value
   equals `{ ok: true, variant: "1060413982" }`. **One assertion only.**

**Fixed interface contract — do not deviate:**

```ts
// routes/api/healthz-smoke-bugfix-1060413982.ts
...
- `routes/api/healthz-smoke-bugfix-1060413982.ts` exists and default-exports a single
  `defineHandler` from `nitro/h3` taking no parameters, matching the contract above exactly.
- `GET /api/healthz-smoke-bugfix-1060413982` on a freshly started server responds with
  `Content-Type: application/json` and the body exactly `{"ok":true,"variant":"1060413982"}` — no
...
- `routes/api/healthz-smoke-bugfix-1060413982.test.ts` exists, constructs an `H3Event` for the
  route's URL, and its single assertion — the handler resolves to
  `{ ok: true, variant: "1060413982" }` — passes. It carries no elapsed-time assertion.
- The production build emits `.output/server/_routes/api/healthz_smoke_bugfix_1060413982.mjs`, and
...
| File                                                 | Action |
| ---------------------------------------------------- | ------ |
| `routes/api/healthz-smoke-bugfix-1060413982.ts`      | create |
| `routes/api/healthz-smoke-bugfix-1060413982.test.ts` | create |
...
for `1060413982` returns zero matches, so the new filename cannot shadow or be shadowed by an
