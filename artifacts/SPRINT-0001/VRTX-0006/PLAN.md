# VRTX-0006 Implementation Plan

**Title**: Implement `/healthz-smoke-136110581-c` endpoint

**Ticket**: VRTX-0006 (TASK)
...
The service needs a lightweight health-check endpoint at `/healthz-smoke-136110581-c` to confirm readiness. No auth, no database, no shared code — just a GET endpoint returning `{ok: true, variant: "136110581"}` with HTTP 200.

## Solution

Implement a standalone Nitro route handler at `routes/api/healthz-smoke-136110581-c.ts` that:

1. Accepts GET requests to `/api/healthz-smoke-136110581-c`
2. Returns `{ok: true, variant: "136110581"}` with HTTP 200
3. Has no dependencies on middleware, database, or other endpoints

## Acceptance Criteria (Definition of Done)

- [ ] File `routes/api/healthz-smoke-136110581-c.ts` created with Nitro handler
- [ ] Handler returns `{ok: true, variant: "136110581"}` with HTTP 200
- [ ] Integration test `routes/api/healthz-smoke-136110581-c.test.ts` created and passing
- [ ] `bun run lint` passes with zero warnings
- [ ] `bun run typecheck` passes
- [ ] `bun run test` passes (all tests including new integration test)
- [ ] Code committed with clear message: "feat(SPRINT-0001): add /healthz-smoke-136110581-c endpoint"

---

## Implementation Details

### File: `routes/api/healthz-smoke-136110581-c.ts`
...
### File: `routes/api/healthz-smoke-136110581-c.test.ts`

```typescript
import { describe, it, expect } from "vitest";
import handler from "./healthz-smoke-136110581-c.ts";
import { createEvent } from "h3";

describe("GET /api/healthz-smoke-136110581-c", () => {
...
1. **Unit/Integration Test**: Vitest + H3Event (routes/api/healthz-smoke-136110581-c.test.ts)
...
2. **Manual Smoke Test** (after all endpoints complete):
   ...
   ```bash
   bun run dev
   curl http://localhost:5000/api/healthz-smoke-136110581-c
...
| ---------------------------------------------- | --------- | ------------------------- |
| `routes/api/healthz-smoke-136110581-c.ts`      | VRTX-0006 | Nitro/H3 (no custom deps) |
| `routes/api/healthz-smoke-136110581-c.test.ts` | VRTX-0006 | Vitest, H3 test utilities |
...
```
GET /api/healthz-smoke-136110581-c
```
...
- [ ] Git commit created with message "feat(SPRINT-0001): add /healthz-smoke-136110581-c endpoint"

---

## Changelog

### 2026-07-26 — VRTX-0006 Implementation

Standalone GET endpoint at `/healthz-smoke-136110581-c` returning `{ok: true, variant: "136110581"}` with HTTP 200. No auth, no database, no shared code. Integration test via Vitest + H3Event. Passes lint, typecheck, and test gates.
