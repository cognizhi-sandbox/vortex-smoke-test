...
`/api/healthz-smoke-bugfix-769466328` returns the SPA shell, should return `ok` + `variant`.

Change: `vrtx3-s-0046-smoke-bugfix-sprint-smoke-b`
Requirement: **Health probe for bugfix variant 769466328** (`specs/health-probes/spec.md`)

## Objective

Serve `GET /api/healthz-smoke-bugfix-769466328` with `200 application/json` and the body
`{"ok":true,"variant":"769466328"}`, plus a colocated regression test.
...
1. Create `routes/api/healthz-smoke-bugfix-769466328.ts` from the pinned template in `design.md`
   § D2, with `variant: "769466328"`. Copy `healthz-smoke-528856326-a.ts` — not a neighbouring
   `bugfix-*` file, for the reason in that section.
2. Create `routes/api/healthz-smoke-bugfix-769466328.test.ts` from the same section's test
...
| File                                                | Action |
| --------------------------------------------------- | ------ |
| `routes/api/healthz-smoke-bugfix-769466328.ts`      | create |
| `routes/api/healthz-smoke-bugfix-769466328.test.ts` | create |

Nothing else. No existing file is modified, and no shared helper, factory, constants module or
barrel export is introduced (`design.md` § D4). Ownership is disjoint from VRTX3-T-0308 and
