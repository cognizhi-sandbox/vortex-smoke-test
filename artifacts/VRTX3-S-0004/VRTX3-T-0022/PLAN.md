...
- `routes/api/healthz-smoke-680958919-a.ts` — endpoint handler
- `routes/api/healthz-smoke-680958919-a.test.ts` — integration tests
...
Add GET endpoint `/api/healthz-smoke-680958919-a` that returns HTTP 200 with JSON body `{"ok":true,"variant":"680958919"}`.
...
Any engineer. No coordination needed with builders of endpoints B and C.

### How

1. Create endpoint file `routes/api/healthz-smoke-680958919-a.ts` using H3Event handler pattern
2. Create test file `routes/api/healthz-smoke-680958919-a.test.ts` with two test cases:
   - Response body matches spec
   - Response time < 100ms
3. Run `bun run test -- healthz-smoke-680958919-a.test.ts` locally to verify
...
- [ ] Endpoint file created: `routes/api/healthz-smoke-680958919-a.ts`
- [ ] Test file created: `routes/api/healthz-smoke-680958919-a.test.ts`
- [ ] No TypeScript errors: `bun run typecheck` passes
- [ ] No lint errors: `bun run lint` passes
- [ ] All tests pass: `bun run test -- healthz-smoke-680958919-a.test.ts` passes

### Functional Correctness

- [ ] GET request to `/api/healthz-smoke-680958919-a` returns HTTP 200
...
- [ ] Test can run independently: `bun run test -- healthz-smoke-680958919-a.test.ts`
...
**File**: `routes/api/healthz-smoke-680958919-a.ts`
...
**File**: `routes/api/healthz-smoke-680958919-a.test.ts`
...
```bash
git add routes/api/healthz-smoke-680958919-a.ts routes/api/healthz-smoke-680958919-a.test.ts
git commit -m "VRTX3-T-0022: Add endpoint /api/healthz-smoke-680958919-a"
```
...
| ---------------------------------------------- | ------------ | ---------------------------- |
| `routes/api/healthz-smoke-680958919-a.ts`      | VRTX3-T-0022 | Endpoint handler (10 lines)  |
| `routes/api/healthz-smoke-680958919-a.test.ts` | VRTX3-T-0022 | Integration tests (25 lines) |
...
import healthzA from "./healthz-smoke-680958919-a.ts";
```
...
const event = new H3Event("http://localhost/api/healthz-smoke-680958919-a");
```
...
- SPRINT-0019: `/api/healthz-smoke-302960562-a` (same pattern, different variant)
...
2. Compare your files to `routes/api/healthz-smoke-302960562-a.ts` + `*.test.ts` (working example)
...
| ---------------------------- | --------------------------------------------------------------------- |
| Endpoint exists and responds | `curl http://localhost:5000/api/healthz-smoke-680958919-a` (dev mode) |
