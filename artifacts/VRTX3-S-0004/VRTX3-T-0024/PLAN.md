# Task Plan — VRTX3-T-0024

**Task**: Endpoint `/api/healthz-smoke-680958919-c`  
**Sprint**: VRTX3-S-0004  
**Story**: VRTX3-T-0021  
...
- `routes/api/healthz-smoke-680958919-c.ts` — endpoint handler
- `routes/api/healthz-smoke-680958919-c.test.ts` — integration tests
...
Add GET endpoint `/api/healthz-smoke-680958919-c` that returns HTTP 200 with JSON body `{"ok":true,"variant":"680958919"}`.
...
Any engineer. No coordination needed with builders of endpoints A and B.

### How

1. Create endpoint file `routes/api/healthz-smoke-680958919-c.ts` using H3Event handler pattern
2. Create test file `routes/api/healthz-smoke-680958919-c.test.ts` with two test cases:
   - Response body matches spec
   - Response time < 100ms
3. Run `bun run test -- healthz-smoke-680958919-c.test.ts` locally to verify
...
- [ ] Endpoint file created: `routes/api/healthz-smoke-680958919-c.ts`
- [ ] Test file created: `routes/api/healthz-smoke-680958919-c.test.ts`
- [ ] No TypeScript errors: `bun run typecheck` passes
- [ ] No lint errors: `bun run lint` passes
- [ ] All tests pass: `bun run test -- healthz-smoke-680958919-c.test.ts` passes

### Functional Correctness

- [ ] GET request to `/api/healthz-smoke-680958919-c` returns HTTP 200
...
- [ ] Test can run independently: `bun run test -- healthz-smoke-680958919-c.test.ts`
...
**File**: `routes/api/healthz-smoke-680958919-c.ts`
...
```bash
git add routes/api/healthz-smoke-680958919-c.ts routes/api/healthz-smoke-680958919-c.test.ts
git commit -m "VRTX3-T-0024: Add endpoint /api/healthz-smoke-680958919-c"
```
...
| ---------------------------------------------- | ------------ | ---------------------------- |
| `routes/api/healthz-smoke-680958919-c.ts`      | VRTX3-T-0024 | Endpoint handler (10 lines)  |
| `routes/api/healthz-smoke-680958919-c.test.ts` | VRTX3-T-0024 | Integration tests (25 lines) |
...
import healthzC from "./healthz-smoke-680958919-c.ts";
```
...
const event = new H3Event("http://localhost/api/healthz-smoke-680958919-c");
```
...
- SPRINT-0019: `/api/healthz-smoke-302960562-c` (same pattern, different variant)
...
2. Compare your files to `routes/api/healthz-smoke-302960562-c.ts` + `*.test.ts` (working example)
...
| ---------------------------- | --------------------------------------------------------------------- |
| Endpoint exists and responds | `curl http://localhost:5000/api/healthz-smoke-680958919-c` (dev mode) |
