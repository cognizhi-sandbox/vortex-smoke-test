...
downstream: [artifacts/VRTX3-S-0034/VRTX3-T-0221/tdd-test-result.md]
---

# Plan — VRTX3-T-0221: Add the missing `/api/healthz-smoke-bugfix-839771954` probe

## Objective

`GET /api/healthz-smoke-bugfix-839771954` answers with `Content-Type: application/json` and the body
`{"ok":true,"variant":"839771954"}`. Today that path is unrouted and is answered by the SPA
...
```
/api/healthz-smoke-bugfix-839771954  →  200 text/html; charset=utf-8   949 B  (SPA shell)
/api/healthz-smoke-528856326-a       →  200 application/json;charset=UTF-8  33 B  {"ok":true,"variant":"528856326"}
```

A repo-wide grep for `839771954` (`*.ts`, `*.tsx`, `*.md`, `*.json`, excluding `node_modules`)
...
1. Create `routes/api/healthz-smoke-bugfix-839771954.ts`: default-export a `defineHandler` imported
   from `nitro/h3`, taking no parameters, returning the literal `{ ok: true, variant: "839771954" }`.
   Copy `routes/api/healthz-smoke-528856326-a.ts` and change only the variant string.
2. Create `routes/api/healthz-smoke-bugfix-839771954.test.ts` by copying
   `routes/api/healthz-smoke-528856326-a.test.ts` — one `it()` case that builds
   `new H3Event(new Request("http://localhost/api/healthz-smoke-bugfix-839771954"))`, awaits the
   imported handler, and asserts `toEqual({ ok: true, variant: "839771954" })`.
3. Check the literal `839771954` agrees in three places: the handler filename, the test filename,
...
- `routes/api/healthz-smoke-bugfix-839771954.ts` (new)
- `routes/api/healthz-smoke-bugfix-839771954.test.ts` (new)

No other file. No overlap with VRTX3-T-0222 or VRTX3-T-0223, so no `depends_on` edge. The root docs
...
```ts
// routes/api/healthz-smoke-bugfix-839771954.ts
...
- Route path: `/api/healthz-smoke-bugfix-839771954` (derived from the filename; the `/api/` prefix
  comes from the `routes/api/` directory and is not optional).
- Response body: exactly `{ ok: true, variant: "839771954" }` — `variant` is the **string**
  `"839771954"`, no extra keys.
...
- DoD-1: `routes/api/healthz-smoke-bugfix-839771954.ts` exists and default-exports a `defineHandler`
  from `nitro/h3` returning `{ ok: true, variant: "839771954" }`.
- DoD-2: `routes/api/healthz-smoke-bugfix-839771954.test.ts` exists, builds a real `H3Event`, calls
  the handler directly, and asserts the exact body. It has exactly one `it()` case.
- DoD-3: A live request to `/api/healthz-smoke-bugfix-839771954` on a running dev server returns
  `Content-Type: application/json` with body `{"ok":true,"variant":"839771954"}` — asserted on the
...
- API-route test, `routes/api/healthz-smoke-bugfix-839771954.test.ts` — covers DoD-1, DoD-2.
