...
**A canvas can reach this section's own conclusion in prose and still name a file it did not check against it.** VRTX3-S-0036's VRTX3-I-0043 named `healthz-smoke-189360772-a` in three sections — a post-VRTX3-S-0011 file, shape-identical to the pinned pair, so the fourth harmless instance against three harmful. What is new is the reasoning that sits beside it: the same canvas **rejects** the timing assertion in its Open Questions, in the same terms this section uses — _"a sub-100ms check on a constant-returning handler measures the runtime, not the code"_ — and then names, as its template, a file it verified only by being a plausible neighbour. Reaching the right conclusion about the assertion is not the same as knowing which of 118 identical-looking files carries it. The two rules that follow are unchanged by it: substitute regardless of how healthy the named file looks, and check which file was named, every time. The governing number is still the 47 legacy tests, now 47 of 118, which are never rewritten.

**A canvas can also get this entirely right — including which specific neighbour to avoid.** VRTX3-S-0037's VRTX3-I-0044 named the pinned `528856326` pair as its template _and_ quoted `healthz-smoke-bugfix3-196651982.ts` as a handler shape example while warning, in its own Fix Shape section, that the same file's **test** is one of the 47 legacy files and must not be copied. Diffed at planning: that test does carry `expect(elapsed).toBeLessThan(100)`, so the warning was accurate. This is the second correct pointer after VRTX3-I-0040 and the first to name a legacy file as a hazard rather than as a template. Two things follow, and neither relaxes the rule above. Quoting a **handler** as a shape example is always safe — handlers carry no timing case, only tests do, so the two halves of a probe pair have different risk and a canvas that cites one has said nothing about the other. And a correct pointer still gets verified: the check that VRTX3-I-0044 was right cost one diff, the same diff that catches a wrong one, so there is no version of this where reading the named file is skippable.

**Two consecutive correct pointers do not retire the check, and the second one shows why.** VRTX3-S-0038's VRTX3-I-0047 named `healthz-smoke-189360772-a.{ts,test.ts}` in its Current State, Technical Approach and Affected Code sections — a post-VRTX3-S-0011 pair carrying no timing case, so the fifth harmless instance against three harmful. It arrived directly after VRTX3-I-0044, the first canvas to name the pinned pair _and_ flag a legacy neighbour by name, which is the closest this family has come to looking like a solved problem. It is not one. VRTX3-I-0047 names the same file VRTX3-I-0043 named three sprints earlier, and it names it for the same reason both times: it is a plausible directory neighbour. The safe outcome is a property of which neighbour got sampled, not of the sampling. The ratio is now **47 of 121** and is fixed by files that are never rewritten.

**Check both halves of a pair, because only one of them can be wrong.** VRTX3-S-0039's VRTX3-I-0048 named `healthz-smoke-1065915107-a.ts` as its handler reference and `healthz-smoke-1065915107-c.test.ts` as its test reference — one post-VRTX3-S-0011 triple, split across two sections, both clean. Sixth harmless instance against three harmful. It is the first canvas to name both halves at once, which makes it the case that exercises the distinction VRTX3-I-0044 introduced: **handlers cannot carry the timing case, only tests can**, so the handler citation was never exposure and the test citation was the entire exposure. A canvas that quotes a handler as a shape example has told you nothing about the pair's risky half — do not read one clean citation as clearing the other. Both were diffed here; that check is one diff per named file and does not get cheaper by looking safe. The ratio moves to **47 of 124**.
...
**Second spec-driven sprint, and the first to extend an existing capability rather than create one.** The change carries three `## ADDED Requirements` against `health-probes` and restates none of the 121 requirements already in the spec of record. Nothing in this manual's four prompt-composed sections changes as a result, and the working instruction is unchanged: your instructions are the ticket's acceptance criteria and its `PLAN.md`, `openspec/` is authored by planning, and `openspec/specs/` is merged by the platform at close.

**The copy-source pointer took a sixth harmless instance — the first to name both halves of a pair.** VRTX3-I-0048 cites `healthz-smoke-1065915107-a.ts` as its handler reference and `healthz-smoke-1065915107-c.test.ts` as its test reference; both are post-VRTX3-S-0011 and both are clean. [Health Probe Routes](#health-probe-routes) records what makes this one instructive rather than routine: only tests can carry the timing case, so the handler citation was never exposure and a clean handler tells you nothing about its sibling test. Both were diffed. Ratio moves to 47 of 124.

**The SPA-fallback measurement reached its twenty-ninth consecutive confirmation, in the quiet form for the second sprint running.** VRTX3-I-0048 makes no status-code claim — only a reproducible "nothing under `routes/api/` currently matches `812788042` (checked)" — and all three paths measured `200 text/html` (949-byte shell) against a control at `200 application/json` (33 bytes) anyway. [Gotchas](#gotchas) carries the entry.

**Closed green: 3/3 tickets DONE, zero defects, 131 test files / 191 tests, full E2E 6 passed.** The sprint also produced the widest dev-server port spread on record — `:5000`, `:5001` and `:5002` across five containers — so the [Gotchas](#gotchas) port entry now says a sprint does not have "a" port at all. Integration QA hit one environment artifact worth knowing: stale dev-server processes left bound to `:5000`–`:5002` by earlier manual verification stalled Playwright's `webServer` boot on `:5178`, which times out at 120 s with no hint about the real cause. Kill stale dev servers before an E2E run even though Playwright uses a different port.

### 2026-08-25 — Sprint VRTX3-S-0038: Three Independent Health Check Endpoints (992401223)

Added `/api/healthz-smoke-992401223-a`, `-b` and `-c`, each returning HTTP 200 with `Content-Type: application/json` and body `{ ok: true, variant: "992401223" }`. Purely additive — 6 new files, 0 existing source files modified. Probe family count 118 → 121, re-derived from the filesystem and bumped in all three docs that carry it (this file, `ARCHITECTURE.md`, `PRODUCT.md`) in the same pass.

**First spec-driven sprint in this repository.** The behaviour contract now lives in an OpenSpec change under `openspec/changes/`, and the sprint plan carries logistics plus a reference to it rather than restating the contract. Nothing in this manual's four prompt-composed sections changes as a result: `openspec/` is authored by planning, `openspec/specs/` is merged by the platform at sprint close, and an implementation agent never writes there. If you are working a probe ticket, your instructions are still the ticket's acceptance criteria and its `PLAN.md`.

**The copy-source pointer took a fifth harmless instance, arriving straight after its cleanest correct one.** VRTX3-I-0047 named `healthz-smoke-189360772-a` — post-VRTX3-S-0011, no timing case — three sprints after VRTX3-I-0043 named the same file for the same reason, and one sprint after VRTX3-I-0044 got the pointer entirely right. [Health Probe Routes](#health-probe-routes) records why two correct-looking sprints in a row are not a reason to relax: the safe outcome is a property of which neighbour was sampled, not of the sampling. Ratio moves to 47 of 121.

**The SPA-fallback measurement reached its twenty-eighth consecutive confirmation, in the quiet form for the fourth time.** VRTX3-I-0047 makes no status-code claim at all — only a reproducible "no file matching `992401223` exists today" — so there was nothing to debunk, and all three paths measured `200 text/html` (949-byte shell) against a control at `200 application/json` (33 bytes) anyway. [Gotchas](#gotchas) carries the entry. The planning container bound `:5000` with no contention.

### 2026-08-23 — Sprint VRTX3-S-0037: Three Missing Health Probes Restored (bugfix 147016547 / 386341015 / 1025161533)

Added `/api/healthz-smoke-bugfix-147016547`, `/api/healthz-smoke-bugfix2-386341015` and `/api/healthz-smoke-bugfix3-1025161533`, each returning HTTP 200 with `Content-Type: application/json` and body `{ ok: true, variant: "<id>" }`. Purely additive — 6 new files, 0 existing source files modified. Probe family count 115 → 118, re-derived from the filesystem and bumped in all three docs that carry it (this file, `ARCHITECTURE.md`, `PRODUCT.md`) in the same pass.

**The copy-source pointer recorded its second fully correct canvas, and the first to name a legacy file as a hazard.** VRTX3-I-0044 named the pinned `528856326` pair as its template while separately warning that `healthz-smoke-bugfix3-196651982.test.ts` — whose handler it quoted as a shape example — is one of the 47 legacy files. Both halves checked out at planning. [Health Probe Routes](#health-probe-routes) now records the distinction that makes the pairing coherent rather than lucky: only tests carry the timing case, so quoting a handler is always safe and says nothing about its sibling test. The governing ratio moves to 47 of 118 and the rule is unchanged — read the named file, every time, including when it turns out to be right.

**The SPA-fallback measurement reached its twenty-seventh consecutive confirmation, and the uneven-capture split its sixth.** VRTX3-I-0044 predicted the `200 text/html` fallback correctly from source and could measure nothing (`:5000`–`:5002` all refused in its capture container); its two sibling defects had no idea linked and asserted `404` unchecked. [Gotchas](#gotchas) now notes the split is six for six with an invariant shape — grounded half predicts and cannot measure, ungrounded half asserts `404` and is wrong — and that neither half observes the working tree.
