...
**The SPA-fallback measurement reached its twenty-ninth consecutive confirmation, in the quiet form for the second sprint running.** VRTX3-I-0048 makes no status-code claim — only a reproducible "nothing under `routes/api/` currently matches `812788042` (checked)" — and all three paths measured `200 text/html` (949-byte shell) against a control at `200 application/json` (33 bytes) anyway. [Gotchas](#gotchas) carries the entry. The planning container reported `Port 5000 is in use` and bound `:5001`.
